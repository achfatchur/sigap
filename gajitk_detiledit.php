...
loadjs.ready("head", function() {
	...
	fgajitk_detiledit.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($gajitk_detil_edit->jp->Required) { ?>
				elm = this.getElements("x" + infix + "_jp");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gajitk_detil_edit->jp->caption(), $gajitk_detil_edit->jp->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_jp");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($gajitk_detil_edit->jp->errorMessage()) ?>");
...
<?php } ?>
<?php if ($gajitk_detil_edit->jp->Visible) { // jp ?>
	<div id="r_jp" class="form-group row">
		<label id="elh_gajitk_detil_jp" for="x_jp" class="<?php echo $gajitk_detil_edit->LeftColumnClass ?>"><?php echo $gajitk_detil_edit->jp->caption() ?><?php echo $gajitk_detil_edit->jp->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $gajitk_detil_edit->RightColumnClass ?>"><div <?php echo $gajitk_detil_edit->jp->cellAttributes() ?>>
<span id="el_gajitk_detil_jp">
<input type="text" data-table="gajitk_detil" data-field="x_jp" name="x_jp" id="x_jp" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($gajitk_detil_edit->jp->getPlaceHolder()) ?>" value="<?php echo $gajitk_detil_edit->jp->EditValue ?>"<?php echo $gajitk_detil_edit->jp->editAttributes() ?>>
</span>
<?php echo $gajitk_detil_edit->jp->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
