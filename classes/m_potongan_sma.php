...
/**
 * Table class for m_potongan_sma
 */
class m_potongan_sma extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_potongan_sma';
		$this->TableName = 'm_potongan_sma';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_potongan_sma`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('m_potongan_sma', 'm_potongan_sma', 'x_id', 'id', '`id`', '`id`', 3, 10, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// tahun
		$this->tahun = new DbField('m_potongan_sma', 'm_potongan_sma', 'x_tahun', 'tahun', '`tahun`', '`tahun`', 3, 10, -1, FALSE, '`tahun`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// bulan
		$this->bulan = new DbField('m_potongan_sma', 'm_potongan_sma', 'x_bulan', 'bulan', '`bulan`', '`bulan`', 3, 10, -1, FALSE, '`bulan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// c_by
		$this->c_by = new DbField('m_potongan_sma', 'm_potongan_sma', 'x_c_by', 'c_by', '`c_by`', '`c_by`', 3, 10, -1, FALSE, '`c_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// datetime
		$this->datetime = new DbField('m_potongan_sma', 'm_potongan_sma', 'x_datetime', 'datetime', '`datetime`', CastDateFieldForLike("`datetime`", 0, "DB"), 135, 19, 0, FALSE, '`datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->datetime->Sortable = TRUE; // Allow sort
		$this->datetime->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['datetime'] = &$this->datetime;

		// import_file
		$this->import_file = new DbField('m_potongan_sma', 'm_potongan_sma', 'x_import_file', 'import_file', '`import_file`', '`import_file`', 201, 65535, -1, TRUE, '`import_file`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "potongan_sma") {
			$detailUrl = $GLOBALS["potongan_sma"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
			$detailUrl .= "&fk_bulan=" . urlencode($this->bulan->CurrentValue);
			$detailUrl .= "&fk_tahun=" . urlencode($this->tahun->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "m_potongan_smalist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_potongan_sma`";
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		// Cascade Update detail table 'potongan_sma'
		$cascadeUpdate = FALSE;
		$rscascade = [];
		...
		if ($cascadeUpdate) {
			...
			$rswrk = $GLOBALS["potongan_sma"]->loadRs("`pid` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB') . " AND " . "`bulan` = " . QuotedValue($rsold['bulan'], DATATYPE_NUMBER, 'DB') . " AND " . "`tahun` = " . QuotedValue($rsold['tahun'], DATATYPE_NUMBER, 'DB'));
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["potongan_sma"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["potongan_sma"]->update($rscascade, $rskey, $rswrk->fields);
				if (!$success)
					return FALSE;

				// Call Row_Updated event
				$GLOBALS["potongan_sma"]->Row_Updated($rsdtlold, $rsdtlnew);
				$rswrk->moveNext();
			}
		}
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		// Cascade delete detail table 'potongan_sma'
		if (!isset($GLOBALS["potongan_sma"]))
			$GLOBALS["potongan_sma"] = new potongan_sma();
		$rscascade = $GLOBALS["potongan_sma"]->loadRs("`pid` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB") . " AND " . "`bulan` = " . QuotedValue($rs['bulan'], DATATYPE_NUMBER, "DB") . " AND " . "`tahun` = " . QuotedValue($rs['tahun'], DATATYPE_NUMBER, "DB"));
		$dtlrows = ($rscascade) ? $rscascade->getRows() : [];

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["potongan_sma"]->Row_Deleting($dtlrow);
			if (!$success)
				break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["potongan_sma"]->delete($dtlrow); // Delete
				if (!$success)
					break;
			}
		}
		...
		if ($success) {
			foreach ($dtlrows as $dtlrow)
				$GLOBALS["potongan_sma"]->Row_Deleted($dtlrow);
		}
		if ($success)
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "m_potongan_smaedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_potongan_smaadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_potongan_smaview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_potongan_smaview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_potongan_smaadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_potongan_smaadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_potongan_smaedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_potongan_smaedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_potongan_smaadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_potongan_smaadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_potongan_smadelete.php", $this->getUrlParm());
	}
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		if($reader) {
			...
			$startRowIdx = 5;
			$records = $worksheet->rangeToArray("A" . $startRowIdx . ":"  . $highestColumn. $highestRow);
			$XLSXdata = "";
			foreach($records as $row) {

				// if ($row[1] == "Total" || isset($row[1]) == FALSE) break; // marker to stop
				// get columns
				...
				.(isset($row[14]) ? "'".$row[14]."'" : "NULL").","
				.(isset($row[15]) ? "'".$row[15]."'" : "NULL").","
				.(isset($row[16]) ? "'".$row[16]."'" : "NULL").","
				.(isset($row[17]) ? "'".$row[17]."'" : "NULL").","
				.(isset($row[18]) ? "'".$row[18]."'" : "NULL").","
				."'".$rsnew["tahun"]."',"
				."'".$rsnew["bulan"]."',"
				."'".$rsnew["id"]."'),";
			}
				if ($XLSXdata != "") {
					$myquery = "INSERT INTO `potongan_sma` VALUES ".rtrim($XLSXdata, ',') . ';';
