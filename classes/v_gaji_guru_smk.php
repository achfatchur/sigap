...
/**
 * Table class for v_gaji_guru_smk
 */
class v_gaji_guru_smk extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'v_gaji_guru_smk';
		$this->TableName = 'v_gaji_guru_smk';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`v_gaji_guru_smk`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// pegawai
		$this->pegawai = new DbField('v_gaji_guru_smk', 'v_gaji_guru_smk', 'x_pegawai', 'pegawai', '`pegawai`', '`pegawai`', 200, 50, -1, FALSE, '`pegawai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// rekbank
		$this->rekbank = new DbField('v_gaji_guru_smk', 'v_gaji_guru_smk', 'x_rekbank', 'rekbank', '`rekbank`', '`rekbank`', 200, 255, -1, FALSE, '`rekbank`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rekbank->Sortable = TRUE; // Allow sort
		$this->fields['rekbank'] = &$this->rekbank;

		// total
		$this->total = new DbField('v_gaji_guru_smk', 'v_gaji_guru_smk', 'x_total', 'total', '`total`', '`total`', 20, 19, -1, FALSE, '`total`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->total->Sortable = TRUE; // Allow sort
		$this->total->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['total'] = &$this->total;

		// potongan
		$this->potongan = new DbField('v_gaji_guru_smk', 'v_gaji_guru_smk', 'x_potongan', 'potongan', '`potongan`', '`potongan`', 20, 19, -1, FALSE, '`potongan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// pid
		$this->pid = new DbField('v_gaji_guru_smk', 'v_gaji_guru_smk', 'x_pid', 'pid', '`pid`', '`pid`', 3, 11, -1, FALSE, '`pid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getMasterFilter()
	{
		...
		// Master filter
		$masterFilter = "";
		if ($this->getCurrentMasterTable() == "v_m_smk") {
	...
	// Master filter
	public function sqlMasterFilter_v_m_smk()
	{
		return "`id`=@id@";
	}

	// Detail filter
	public function sqlDetailFilter_v_m_smk()
	{
		return "`pid`=@pid@";
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`v_gaji_guru_smk`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "v_gaji_guru_smklist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "v_gaji_guru_smkview.php")
			return $Language->phrase("View");
		elseif ($pageName == "v_gaji_guru_smkedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "v_gaji_guru_smkadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("v_gaji_guru_smkview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("v_gaji_guru_smkview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "v_gaji_guru_smkadd.php?" . $this->getUrlParm($parm);
		else
			$url = "v_gaji_guru_smkadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v_gaji_guru_smkedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v_gaji_guru_smkadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("v_gaji_guru_smkdelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "v_m_smk" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
