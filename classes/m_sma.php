...
/**
 * Table class for m_sma
 */
class m_sma extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_sma';
		$this->TableName = 'm_sma';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_sma`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('m_sma', 'm_sma', 'x_id', 'id', '`id`', '`id`', 3, 10, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// datetime
		$this->datetime = new DbField('m_sma', 'm_sma', 'x_datetime', 'datetime', '`datetime`', CastDateFieldForLike("`datetime`", 0, "DB"), 135, 19, 0, FALSE, '`datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->datetime->Sortable = TRUE; // Allow sort
		$this->datetime->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['datetime'] = &$this->datetime;

		// createby
		$this->createby = new DbField('m_sma', 'm_sma', 'x_createby', 'createby', '`createby`', '`createby`', 3, 10, -1, FALSE, '`createby`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tahun
		$this->tahun = new DbField('m_sma', 'm_sma', 'x_tahun', 'tahun', '`tahun`', '`tahun`', 3, 11, -1, FALSE, '`tahun`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// bulan
		$this->bulan = new DbField('m_sma', 'm_sma', 'x_bulan', 'bulan', '`bulan`', '`bulan`', 3, 10, -1, FALSE, '`bulan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "gaji_sma") {
			$detailUrl = $GLOBALS["gaji_sma"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_bulan=" . urlencode($this->bulan->CurrentValue);
			$detailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
			$detailUrl .= "&fk_tahun=" . urlencode($this->tahun->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "m_smalist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_sma`";
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		// Cascade Update detail table 'gaji_sma'
		$cascadeUpdate = FALSE;
		$rscascade = [];
		...
		if ($cascadeUpdate) {
			if (!isset($GLOBALS["gaji_sma"]))
				$GLOBALS["gaji_sma"] = new gaji_sma();
			$rswrk = $GLOBALS["gaji_sma"]->loadRs("`bulan` = " . QuotedValue($rsold['bulan'], DATATYPE_NUMBER, 'DB') . " AND " . "`pid` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB') . " AND " . "`tahun` = " . QuotedValue($rsold['tahun'], DATATYPE_NUMBER, 'DB'));
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["gaji_sma"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["gaji_sma"]->update($rscascade, $rskey, $rswrk->fields);
				if (!$success)
					return FALSE;

				// Call Row_Updated event
				$GLOBALS["gaji_sma"]->Row_Updated($rsdtlold, $rsdtlnew);
				$rswrk->moveNext();
			}
		}
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		// Cascade delete detail table 'gaji_sma'
		if (!isset($GLOBALS["gaji_sma"]))
			$GLOBALS["gaji_sma"] = new gaji_sma();
		$rscascade = $GLOBALS["gaji_sma"]->loadRs("`bulan` = " . QuotedValue($rs['bulan'], DATATYPE_NUMBER, "DB") . " AND " . "`pid` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB") . " AND " . "`tahun` = " . QuotedValue($rs['tahun'], DATATYPE_NUMBER, "DB"));
		$dtlrows = ($rscascade) ? $rscascade->getRows() : [];

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["gaji_sma"]->Row_Deleting($dtlrow);
			if (!$success)
				break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["gaji_sma"]->delete($dtlrow); // Delete
				if (!$success)
					break;
			}
		}
		...
		if ($success) {
			foreach ($dtlrows as $dtlrow)
				$GLOBALS["gaji_sma"]->Row_Deleted($dtlrow);
		}
		if ($success)
	...
	{
		global $Language;
		if ($pageName == "m_smaview.php")
			return $Language->phrase("View");
		elseif ($pageName == "m_smaedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_smaadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_smaview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_smaview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_smaadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_smaadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_smaedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_smaedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_smaadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_smaadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_smadelete.php", $this->getUrlParm());
	}
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//echo "Row Inserted"
			...
			$tahun = $rsnew["tahun"];
			$bulan = $rsnew["bulan"];
			$row = ExecuteRows("SELECT * FROM pegawai where jenjang_id ='4' AND `type`= '1'");
			...
			//die;
				...
				$myquery = "INSERT INTO gaji_sma VALUES (NULL,'".$query["nip"]."','".date('Y-m-d H:i:s')."','".date('Y-m-d')."',NULL,NULL,'".$query["jabatan"]."',NULL,'".$total."',NULL,NULL,NULL,NULL,NULL,NULL,NULL,'".$sub_total."','".$penyesuaian."','4','".$tambahan."','".$id."',NULL,NULL,NULL,NULL,NULL,NULL,NULL, NULL,NULL, NULL, '".$tahun."', '".$bulan."', NULL)";
			...
			$bulan_query = ExecuteRows("SELECT * FROM m_semeter where detil_smtr ='$bulan'");
				...
				$myquery = "INSERT INTO m_sma VALUES (NULL,'".$tahun."','".$row["bulan"]."',NULL,NULL)";
				//print_r($myquery);
				//die;
				$myResult = Execute($myquery);*/
		}			
	...
	function Row_Deleting(&$rs) {
		...
		$bulan = $rs["bulan"];
		$id = $rs["id"];
		$delete = ("DELETE FROM generate_pertahun WHERE tahun ='".$tahun."' AND bulan ='".$bulan."' AND profesi ='1'");
		$delete_detil = ("DELETE FROM gaji_sma WHERE pid='".$id."'");
		return TRUE;
	}
	// Row Deleted event
