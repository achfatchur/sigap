...
/**
 * Table class for v_gajisma
 */
class v_gajisma extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'v_gajisma';
		$this->TableName = 'v_gajisma';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`v_gajisma`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// pid
		$this->pid = new DbField('v_gajisma', 'v_gajisma', 'x_pid', 'pid', '`pid`', '`pid`', 3, 11, -1, FALSE, '`pid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// pegawai
		$this->pegawai = new DbField('v_gajisma', 'v_gajisma', 'x_pegawai', 'pegawai', '`pegawai`', '`pegawai`', 200, 50, -1, FALSE, '`pegawai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// rekbank
		$this->rekbank = new DbField('v_gajisma', 'v_gajisma', 'x_rekbank', 'rekbank', '`rekbank`', '`rekbank`', 200, 255, -1, FALSE, '`rekbank`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rekbank->Sortable = TRUE; // Allow sort
		$this->fields['rekbank'] = &$this->rekbank;

		// total
		$this->total = new DbField('v_gajisma', 'v_gajisma', 'x_total', 'total', '`total`', '`total`', 20, 19, -1, FALSE, '`total`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->total->Sortable = TRUE; // Allow sort
		$this->total->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['total'] = &$this->total;

		// potongan
		$this->potongan = new DbField('v_gajisma', 'v_gajisma', 'x_potongan', 'potongan', '`potongan`', '`potongan`', 20, 19, -1, FALSE, '`potongan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getMasterFilter()
	{
		...
		// Master filter
		$masterFilter = "";
		if ($this->getCurrentMasterTable() == "v_m_sma") {
	...
	// Master filter
	public function sqlMasterFilter_v_m_sma()
	{
		return "`id`=@id@";
	}

	// Detail filter
	public function sqlDetailFilter_v_m_sma()
	{
		return "`pid`=@pid@";
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`v_gajisma`";
	}
	...
	{
		global $Language;
		if ($pageName == "v_gajismaview.php")
			return $Language->phrase("View");
		elseif ($pageName == "v_gajismaedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "v_gajismaadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("v_gajismaview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("v_gajismaview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "v_gajismaadd.php?" . $this->getUrlParm($parm);
		else
			$url = "v_gajismaadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v_gajismaedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v_gajismaadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("v_gajismadelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "v_m_sma" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
