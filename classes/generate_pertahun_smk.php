...
/**
 * Table class for generate_pertahun_smk
 */
class generate_pertahun_smk extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'generate_pertahun_smk';
		$this->TableName = 'generate_pertahun_smk';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`generate_pertahun_smk`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('generate_pertahun_smk', 'generate_pertahun_smk', 'x_id', 'id', '`id`', '`id`', 3, 10, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// profesi
		$this->profesi = new DbField('generate_pertahun_smk', 'generate_pertahun_smk', 'x_profesi', 'profesi', '`profesi`', '`profesi`', 3, 10, -1, FALSE, '`profesi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// tahun
		$this->tahun = new DbField('generate_pertahun_smk', 'generate_pertahun_smk', 'x_tahun', 'tahun', '`tahun`', '`tahun`', 3, 10, -1, FALSE, '`tahun`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// bulan
		$this->bulan = new DbField('generate_pertahun_smk', 'generate_pertahun_smk', 'x_bulan', 'bulan', '`bulan`', '`bulan`', 200, 50, -1, FALSE, '`bulan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->bulan->Sortable = TRUE; // Allow sort
		$this->bulan->Lookup = new Lookup('bulan', 'bulan', FALSE, 'id', ["bulan","","",""], [], [], [], [], [], [], '', '');
		$this->fields['bulan'] = &$this->bulan;

		// bulan2
		$this->bulan2 = new DbField('generate_pertahun_smk', 'generate_pertahun_smk', 'x_bulan2', 'bulan2', '`bulan2`', '`bulan2`', 3, 10, -1, FALSE, '`bulan2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`generate_pertahun_smk`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "generate_pertahun_smklist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "generate_pertahun_smkview.php")
			return $Language->phrase("View");
		elseif ($pageName == "generate_pertahun_smkedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "generate_pertahun_smkadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("generate_pertahun_smkview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("generate_pertahun_smkview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "generate_pertahun_smkadd.php?" . $this->getUrlParm($parm);
		else
			$url = "generate_pertahun_smkadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("generate_pertahun_smkedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("generate_pertahun_smkadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("generate_pertahun_smkdelete.php", $this->getUrlParm());
	}
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//echo "Row Inserted"
			...
			//	foreach ($bulan_query as $row) {
					...
					$querymy2 = "INSERT INTO generate_pertahun_smk VALUES(NULL,'".$tahun."',NULL,NULL,'".$bulan."')";
					$Result2 = Execute($querymy2);

					//delete field bulan ketika generate bulan2
					$delete ="DELETE FROM generate_pertahun_smk WHERE bulan2 is null";
					$delete2 = Execute($delete);
					$delete_clone ="DELETE FROM generate_pertahun_smk WHERE id < '".$id."' AND tahun ='".$tahun."' AND bulan2='".$bulan."'";	
					...
					//if($this->profesi->CurrentValue == '1'){
						$delete_detil_karyawan ="DELETE FROM gaji_smk WHERE tahun ='".$tahun."' AND bulan='".$bulan."'";	
						$detil_delete = execute($delete_detil_karyawan);			
						$delete_m_sd ="DELETE FROM m_smk WHERE tahun ='".$tahun."' AND bulan='".$bulan."' ORDER BY id asc";	
						$clone_delete = execute($delete_m_sd);

						//$querymy2 = "INSERT INTO m_sma VALUES (NULL,'".$tahun."',NULL,'".$bulan."')";
						//$Result2 = Execute($querymy2);	

						$querymy = "INSERT INTO m_smk VALUES (NULL,'".$tahun."','".$bulan."',NULL,NULL)";
						$Result = Execute($querymy);

						// print_r($querymy);
						// die;

						$pid = ExecuteScalar("select id from m_smk order by id DESC limit 1");
						$row2 = ExecuteRows("SELECT * FROM pegawai where jenjang_id ='5' AND `type`= '1'");
					...
					foreach($row2 as $query){
						...
						$piket= ExecuteRow("select * from m_piket where jenjang='5' AND jenis_sertif ='".$query['sertif']."' AND type_jabatan='1'");

						//value kehadiran
						$kehadiran=ExecuteRow("Select * from m_kehadiran where jenjang='5' AND jenis_jabatan='1' AND sertif='".$query['sertif']."'");	

						//print_r($kehadiran);
						//die;																		

						$tunjanagan_tambahan=ExecuteRow("select * from tunjangan_tambahan where jenjang='5' AND kualifikasi='".$query['tambahan']."'");
				...
				//}
						...
						$gaji_pokok=ExecuteRow("SELECT * FROM gaji_pokok WHERE jenjang_id='5' AND lama_kerja='".$query['lama_kerja']."'");
				...
				//$komponen_gapok= ($gaji_pokok["value"] * $query["jjm"]);
					...
					//}elseif($this->profesi->CurrentValue == '2'){
						$delete_detil_karyawan ="DELETE FROM gaji_tu_smk WHERE tahun ='".$tahun."' AND bulan='".$bulan."'";	
						$detil_delete = execute($delete_detil_karyawan);			
						$delete_m_sd ="DELETE FROM m_tu_smk WHERE tahun ='".$tahun."' AND bulan='".$bulan."' ORDER BY id asc";	
						$clone_delete = execute($delete_m_sd);
							$querymy = "INSERT INTO m_tu_smk VALUES (NULL,'".$tahun."','".$bulan."',NULL,NULL)";
							$Result = Execute($querymy);
							$pid = ExecuteScalar("select id from m_tu_smk order by id DESC limit 1");
							$row2 = ExecuteRows("SELECT * FROM pegawai where jenjang_id ='5' AND `type`= '2'");
					...
					foreach($row2 as $query){
						...
						//$kehadiran=ExecuteRow("Select * from m_kehadiran where jenjang='5' AND jenis_jabatan='1'");	
						$piket= ExecuteRow("select * from m_piket where jenjang='5' AND type_jabatan='2'");	
						$kehadiran=ExecuteRow("Select * from m_kehadiran where jenjang='5' AND jenis_jabatan='2'");																		
						$tunjanagan_tambahan=ExecuteRow("select * from tunjangan_tambahan where jenjang='5' AND kualifikasi='".$query['tambahan']."'");
						$absen =ExecuteRow("SELECT * FROM penyesuaian INNER JOIN m_penyesuaian ON m_penyesuaian.id = penyesuaian.m_id WHERE penyesuaian.nip='".$query['nip']."' AND  m_penyesuaian.tahun='".$tahun."' AND m_penyesuaian.bulan='".$bulan."' ORDER BY m_penyesuaian.id DESC");	
						...
						$lembur = ExecuteScalar("SELECT value_perjam FROM m_lembur WHERE jenjang_id='".$query["jenjang_id"]."'");
						$gaji_pokok=ExecuteRow("SELECT * FROM gaji_pokok_tu WHERE jenjang_id='5' AND lama_kerja='".$query['lama_kerja']."' AND ijazah= '".$query['pendidikan']."'");
				...
				//	}else{
						$delete_detil_karyawan ="DELETE FROM gaji_karyawan_smk WHERE tahun ='".$tahun."' AND bulan='".$bulan."'";	
						$detil_delete = execute($delete_detil_karyawan);			
						$delete_m_sd ="DELETE FROM m_karyawan_smk WHERE tahun ='".$tahun."' AND bulan='".$bulan."' ORDER BY id asc";	
						$clone_delete = execute($delete_m_sd);
						$querymy = "INSERT INTO m_karyawan_smk VALUES (NULL,'".$tahun."','".$bulan."',NULL,NULL)";
						$Result = Execute($querymy);
						$pid = ExecuteScalar("select id from m_karyawan_smk order by id DESC limit 1");
						$row2 = ExecuteRows("SELECT * FROM pegawai where jenjang_id ='5' AND `type`= '3'");	
					foreach($row2 as $query){
					...
					$kehadiran= ExecuteRow("Select * from m_kehadiran where jenjang='5' AND jabatan= '".$query["jabatan"]."'");	

						//GAJI POKOK
					$query2 = ExecuteRow("SELECT * FROM jabatan JOIN gajitunjangan ON jabatan.id = gajitunjangan.pidjabatan where gajitunjangan.pidjabatan='9'");
					$querypokok = ExecuteRow("SELECT * FROM gaji_pokok_kayawan where jenjang_id='5' AND jabatan_id= '".$query["jabatan"]."'");		
					$inval = ExecuteScalar("SELECT value FROM m_inval WHERE jenjang_id='".$query["jenjang_id"]."'");
					$lembur = ExecuteScalar("SELECT value_perjam FROM m_lembur WHERE jenjang_id='".$query["jenjang_id"]."'");							
					$piket= ExecuteRow("select * from m_piket where jenjang='5' AND type_jabatan='3'");

					//Gaji Pokok	
					$kehadiran = ExecuteRow("select * from m_kehadiran where jenjang ='5' AND jenis_jabatan='3' AND jabatan='".$query["jabatan"]."'");
					$gaji_pokok=ExecuteRow("SELECT * FROM gaji_pokok_kayawan WHERE jenjang_id='5' AND jabatan_id= '".$query["jabatan"]."'");	
					$absen =ExecuteRow("SELECT * FROM penyesuaian INNER JOIN m_penyesuaian ON m_penyesuaian.id = penyesuaian.m_id WHERE penyesuaian.nip='".$query['nip']."' AND  m_penyesuaian.tahun='".$tahun."' AND m_penyesuaian.bulan='".$bulan."' ORDER BY m_penyesuaian.id DESC");	
					...
					$myquery2 = "INSERT INTO gaji_karyawan_smk VALUES (NULL, '".$query["nip"]."','".$query["jabatan"]."','4','".$komponen_gapok."',NULL,'".$inval2."','".$c_jjm."','".$sub_total."','".$penyesuaian."','".$tambahan."','".$total."','".$pid."','".$v_kehadiran."',NULL,'".$tahun."', '".$bulan."','".$v_voucher."',NULL,'".$value_pensiun."','".$value_hari_tua."','".$pph21."','".$solve_bpjs."','".$solved_npwp."')";
