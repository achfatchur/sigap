...
/**
 * Table class for m_kehadiran
 */
class m_kehadiran extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_kehadiran';
		$this->TableName = 'm_kehadiran';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_kehadiran`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('m_kehadiran', 'm_kehadiran', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// jenjang
		$this->jenjang = new DbField('m_kehadiran', 'm_kehadiran', 'x_jenjang', 'jenjang', '`jenjang`', '`jenjang`', 3, 11, -1, FALSE, '`jenjang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// jenis_jabatan
		$this->jenis_jabatan = new DbField('m_kehadiran', 'm_kehadiran', 'x_jenis_jabatan', 'jenis_jabatan', '`jenis_jabatan`', '`jenis_jabatan`', 3, 11, -1, FALSE, '`jenis_jabatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->jenis_jabatan->Sortable = TRUE; // Allow sort
		$this->jenis_jabatan->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->jenis_jabatan->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
		$this->jenis_jabatan->Lookup = new Lookup('jenis_jabatan', 'jenis_jabatan', FALSE, 'id', ["name","","",""], [], [], [], [], [], [], '', '');
		$this->jenis_jabatan->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['jenis_jabatan'] = &$this->jenis_jabatan;

		// jabatan
		$this->jabatan = new DbField('m_kehadiran', 'm_kehadiran', 'x_jabatan', 'jabatan', '`jabatan`', '`jabatan`', 3, 11, -1, FALSE, '`jabatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->jabatan->Lookup = new Lookup('jabatan', 'jabatan', FALSE, 'id', ["nama_jabatan","","",""], [], [], [], [], [], [], '', '');
		$this->jabatan->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['jabatan'] = &$this->jabatan;

		// sertif
		$this->sertif = new DbField('m_kehadiran', 'm_kehadiran', 'x_sertif', 'sertif', '`sertif`', '`sertif`', 3, 11, -1, FALSE, '`sertif`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// value
		$this->value = new DbField('m_kehadiran', 'm_kehadiran', 'x_value', 'value', '`value`', '`value`', 20, 20, -1, FALSE, '`value`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_kehadiran`";
	}
	...
	{
		global $Language;
		if ($pageName == "m_kehadiranview.php")
			return $Language->phrase("View");
		elseif ($pageName == "m_kehadiranedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_kehadiranadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_kehadiranview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_kehadiranview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_kehadiranadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_kehadiranadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_kehadiranedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_kehadiranadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_kehadirandelete.php", $this->getUrlParm());
	}
	...
	public function renderListRow()
	{
		...
		// jenis_jabatan
		$curVal = strval($this->jenis_jabatan->CurrentValue);
		if ($curVal != "") {
			$this->jenis_jabatan->ViewValue = $this->jenis_jabatan->lookupCacheOption($curVal);
			if ($this->jenis_jabatan->ViewValue === NULL) { // Lookup from database
				$filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
				$sqlWrk = $this->jenis_jabatan->Lookup->getSql(FALSE, $filterWrk, '', $this);
				$rswrk = Conn()->execute($sqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$arwrk = [];
					$arwrk[1] = $rswrk->fields('df');
					$this->jenis_jabatan->ViewValue = $this->jenis_jabatan->displayValue($arwrk);
					$rswrk->Close();
				} else {
			...
			$this->jenis_jabatan->ViewValue = NULL;
		}
