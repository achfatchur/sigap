...
class jabatan extends DbTable
{
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('jabatan', 'jabatan', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// nama_jabatan
		$this->nama_jabatan = new DbField('jabatan', 'jabatan', 'x_nama_jabatan', 'nama_jabatan', '`nama_jabatan`', '`nama_jabatan`', 200, 255, -1, FALSE, '`nama_jabatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama_jabatan->Sortable = TRUE; // Allow sort
		$this->fields['nama_jabatan'] = &$this->nama_jabatan;

		// type_jabatan
		$this->type_jabatan = new DbField('jabatan', 'jabatan', 'x_type_jabatan', 'type_jabatan', '`type_jabatan`', '`type_jabatan`', 3, 11, -1, FALSE, '`type_jabatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->type_jabatan->Sortable = TRUE; // Allow sort
		$this->type_jabatan->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->type_jabatan->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
		...
		// jenjang
		$this->jenjang = new DbField('jabatan', 'jabatan', 'x_jenjang', 'jenjang', '`jenjang`', '`jenjang`', 3, 11, -1, FALSE, '`jenjang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->jenjang->Sortable = TRUE; // Allow sort
		$this->jenjang->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->jenjang->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
		...
		// keterangan
		$this->keterangan = new DbField('jabatan', 'jabatan', 'x_keterangan', 'keterangan', '`keterangan`', '`keterangan`', 200, 255, -1, FALSE, '`keterangan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// c_date
		$this->c_date = new DbField('jabatan', 'jabatan', 'x_c_date', 'c_date', '`c_date`', CastDateFieldForLike("`c_date`", 0, "DB"), 135, 19, 0, FALSE, '`c_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// u_date
		$this->u_date = new DbField('jabatan', 'jabatan', 'x_u_date', 'u_date', '`u_date`', CastDateFieldForLike("`u_date`", 0, "DB"), 135, 19, 0, FALSE, '`u_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_date->Sortable = TRUE; // Allow sort
		...
		// aktif
		$this->aktif = new DbField('jabatan', 'jabatan', 'x_aktif', 'aktif', '`aktif`', '`aktif`', 16, 4, -1, FALSE, '`aktif`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "gajitunjangan") {
			$detailUrl = $GLOBALS["gajitunjangan"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "jabatanlist.php";
		return $detailUrl;
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		// Cascade Update detail table 'gajitunjangan'
		$cascadeUpdate = FALSE;
		$rscascade = [];
		if ($rsold && (isset($rs['id']) && $rsold['id'] != $rs['id'])) { // Update detail field 'pidjabatan'
			$cascadeUpdate = TRUE;
			$rscascade['pidjabatan'] = $rs['id'];
		}
		if ($cascadeUpdate) {
			if (!isset($GLOBALS["gajitunjangan"]))
				$GLOBALS["gajitunjangan"] = new gajitunjangan();
			$rswrk = $GLOBALS["gajitunjangan"]->loadRs("`pidjabatan` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB'));
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["gajitunjangan"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["gajitunjangan"]->update($rscascade, $rskey, $rswrk->fields);
				if (!$success)
					return FALSE;

				// Call Row_Updated event
				$GLOBALS["gajitunjangan"]->Row_Updated($rsdtlold, $rsdtlnew);
				$rswrk->moveNext();
			}
		}
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		// Cascade delete detail table 'gajitunjangan'
		if (!isset($GLOBALS["gajitunjangan"]))
			$GLOBALS["gajitunjangan"] = new gajitunjangan();
		$rscascade = $GLOBALS["gajitunjangan"]->loadRs("`pidjabatan` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB"));
		$dtlrows = ($rscascade) ? $rscascade->getRows() : [];

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["gajitunjangan"]->Row_Deleting($dtlrow);
			if (!$success)
				break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["gajitunjangan"]->delete($dtlrow); // Delete
				if (!$success)
					break;
			}
		}
		...
		if ($success) {
			foreach ($dtlrows as $dtlrow)
				$GLOBALS["gajitunjangan"]->Row_Deleted($dtlrow);
		}
		if ($success)
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->type_guru->DbValue = $row['type_guru'];
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("jabatanview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("jabatanview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "jabatanadd.php?" . $this->getUrlParm($parm);
		else
			$url = "jabatanadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("jabatanedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("jabatanedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("jabatanadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("jabatanadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("jabatandelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->type_guru->setDbValue($rs->fields('type_guru'));
	...
	public function renderEditRow()
	{
		...
		$this->type_guru->EditValue = $this->type_guru->CurrentValue;
