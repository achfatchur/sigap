...
/**
 * Table class for peg_dokumen
 */
class peg_dokumen extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'peg_dokumen';
		$this->TableName = 'peg_dokumen';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`peg_dokumen`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('peg_dokumen', 'peg_dokumen', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// pid
		$this->pid = new DbField('peg_dokumen', 'peg_dokumen', 'x_pid', 'pid', '`pid`', '`pid`', 3, 11, -1, FALSE, '`pid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->pid->Sortable = TRUE; // Allow sort
		$this->pid->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['pid'] = &$this->pid;

		// nama_dokumen
		$this->nama_dokumen = new DbField('peg_dokumen', 'peg_dokumen', 'x_nama_dokumen', 'nama_dokumen', '`nama_dokumen`', '`nama_dokumen`', 200, 255, -1, FALSE, '`nama_dokumen`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama_dokumen->Sortable = TRUE; // Allow sort
		$this->fields['nama_dokumen'] = &$this->nama_dokumen;

		// file_dokumen
		$this->file_dokumen = new DbField('peg_dokumen', 'peg_dokumen', 'x_file_dokumen', 'file_dokumen', '`file_dokumen`', '`file_dokumen`', 200, 255, -1, TRUE, '`file_dokumen`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
		$this->file_dokumen->Sortable = TRUE; // Allow sort
		$this->fields['file_dokumen'] = &$this->file_dokumen;

		// keterangan
		$this->keterangan = new DbField('peg_dokumen', 'peg_dokumen', 'x_keterangan', 'keterangan', '`keterangan`', '`keterangan`', 200, 255, -1, FALSE, '`keterangan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->keterangan->Sortable = TRUE; // Allow sort
		$this->fields['keterangan'] = &$this->keterangan;

		// c_date
		$this->c_date = new DbField('peg_dokumen', 'peg_dokumen', 'x_c_date', 'c_date', '`c_date`', CastDateFieldForLike("`c_date`", 0, "DB"), 135, 19, 0, FALSE, '`c_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// u_date
		$this->u_date = new DbField('peg_dokumen', 'peg_dokumen', 'x_u_date', 'u_date', '`u_date`', CastDateFieldForLike("`u_date`", 0, "DB"), 135, 19, 0, FALSE, '`u_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// c_by
		$this->c_by = new DbField('peg_dokumen', 'peg_dokumen', 'x_c_by', 'c_by', '`c_by`', '`c_by`', 3, 11, -1, FALSE, '`c_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// u_by
		$this->u_by = new DbField('peg_dokumen', 'peg_dokumen', 'x_u_by', 'u_by', '`u_by`', '`u_by`', 3, 11, -1, FALSE, '`u_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`peg_dokumen`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->nama_dokumen->DbValue = $row['nama_dokumen'];
	...
	public function deleteUploadedFiles($row)
	{
		...
		$oldFiles = EmptyValue($row['file_dokumen']) ? [] : [$row['file_dokumen']];
		foreach ($oldFiles as $oldFile) {
			if (file_exists($this->file_dokumen->oldPhysicalUploadPath() . $oldFile))
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "peg_dokumenedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "peg_dokumenadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("peg_dokumenview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("peg_dokumenview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "peg_dokumenadd.php?" . $this->getUrlParm($parm);
		else
			$url = "peg_dokumenadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("peg_dokumenedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("peg_dokumenadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("peg_dokumendelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->nama_dokumen->setDbValue($rs->fields('nama_dokumen'));
		$this->file_dokumen->Upload->DbValue = $rs->fields('file_dokumen');
	...
	public function renderEditRow()
	{
		...
		$this->nama_dokumen->EditValue = $this->nama_dokumen->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->nama_dokumen);
					$doc->exportCaption($this->file_dokumen);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->nama_dokumen);
						$doc->exportField($this->file_dokumen);
	...
	public function getFileData($fldparm, $key, $resize, $width = 0, $height = 0)
	{
		...
		$fileNameFld = "";
		$fileTypeFld = "";
		if ($fldparm == 'file_dokumen') {
			$fldName = "file_dokumen";
			$fileNameFld = "file_dokumen";
		} else {
