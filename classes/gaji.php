...
class gaji extends DbTable
{
	...
	public function __construct()
	{
		...
		// datetime
		$this->datetime = new DbField('gaji', 'gaji', 'x_datetime', 'datetime', '`datetime`', CastDateFieldForLike("`datetime`", 0, "DB"), 135, 19, 0, FALSE, '`datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->datetime->Sortable = TRUE; // Allow sort
		$this->datetime->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['datetime'] = &$this->datetime;

		// pegawai
		$this->pegawai = new DbField('gaji', 'gaji', 'x_pegawai', 'pegawai', '`pegawai`', '`pegawai`', 200, 50, -1, FALSE, '`pegawai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// month
		$this->month = new DbField('gaji', 'gaji', 'x_month', 'month', '`month`', CastDateFieldForLike("`month`", 0, "DB"), 133, 10, 0, FALSE, '`month`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// status
		$this->status = new DbField('gaji', 'gaji', 'x_status', 'status', '`status`', '`status`', 200, 50, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	// Master filter
	public function sqlMasterFilter_m_sd()
	{
		return "`id`=@id@ AND `tahun`=@tahun@ AND `bulan`=@bulan@";
	}

	// Detail filter
	public function sqlDetailFilter_m_sd()
	{
	...
	{
		global $Language;
		if ($pageName == "gajiview.php")
			return $Language->phrase("View");
		elseif ($pageName == "gajiedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "gajiadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("gajiview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("gajiview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "gajiadd.php?" . $this->getUrlParm($parm);
		else
			$url = "gajiadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("gajiedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("gajiadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("gajidelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "m_sd" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
		...
		if (CurrentPageID() == "add") {
						...
						if(isset($this->RowIndex)) {
							...
							if(($grid_count >= $grid_num) && is_int($grid_num) && ($grid_num >= 1)){
								$offseter = $grid_num - 1;
								$row = ExecuteRow("SELECT * FROM pegawai where jenjang_id ='2' AND `type`= '1' limit 1 OFFSET $offseter");
