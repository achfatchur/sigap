...
/**
 * Table class for m_tu_sma
 */
class m_tu_sma extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_tu_sma';
		$this->TableName = 'm_tu_sma';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_tu_sma`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('m_tu_sma', 'm_tu_sma', 'x_id', 'id', '`id`', '`id`', 3, 10, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// datetime
		$this->datetime = new DbField('m_tu_sma', 'm_tu_sma', 'x_datetime', 'datetime', '`datetime`', CastDateFieldForLike("`datetime`", 0, "DB"), 135, 19, 0, FALSE, '`datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->datetime->Sortable = TRUE; // Allow sort
		$this->datetime->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['datetime'] = &$this->datetime;

		// createby
		$this->createby = new DbField('m_tu_sma', 'm_tu_sma', 'x_createby', 'createby', '`createby`', '`createby`', 3, 10, -1, FALSE, '`createby`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tahun
		$this->tahun = new DbField('m_tu_sma', 'm_tu_sma', 'x_tahun', 'tahun', '`tahun`', '`tahun`', 18, 4, -1, FALSE, '`tahun`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// bulan
		$this->bulan = new DbField('m_tu_sma', 'm_tu_sma', 'x_bulan', 'bulan', '`bulan`', '`bulan`', 3, 10, -1, FALSE, '`bulan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "gaji_tu_sma") {
			$detailUrl = $GLOBALS["gaji_tu_sma"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
			$detailUrl .= "&fk_tahun=" . urlencode($this->tahun->CurrentValue);
			$detailUrl .= "&fk_bulan=" . urlencode($this->bulan->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "m_tu_smalist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_tu_sma`";
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		// Cascade Update detail table 'gaji_tu_sma'
		$cascadeUpdate = FALSE;
		$rscascade = [];
		...
		if ($cascadeUpdate) {
			if (!isset($GLOBALS["gaji_tu_sma"]))
				$GLOBALS["gaji_tu_sma"] = new gaji_tu_sma();
			$rswrk = $GLOBALS["gaji_tu_sma"]->loadRs("`pid` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB') . " AND " . "`tahun` = " . QuotedValue($rsold['tahun'], DATATYPE_NUMBER, 'DB') . " AND " . "`bulan` = " . QuotedValue($rsold['bulan'], DATATYPE_NUMBER, 'DB'));
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["gaji_tu_sma"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["gaji_tu_sma"]->update($rscascade, $rskey, $rswrk->fields);
				if (!$success)
					return FALSE;

				// Call Row_Updated event
				$GLOBALS["gaji_tu_sma"]->Row_Updated($rsdtlold, $rsdtlnew);
				$rswrk->moveNext();
			}
		}
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		// Cascade delete detail table 'gaji_tu_sma'
		if (!isset($GLOBALS["gaji_tu_sma"]))
			$GLOBALS["gaji_tu_sma"] = new gaji_tu_sma();
		$rscascade = $GLOBALS["gaji_tu_sma"]->loadRs("`pid` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB") . " AND " . "`tahun` = " . QuotedValue($rs['tahun'], DATATYPE_NUMBER, "DB") . " AND " . "`bulan` = " . QuotedValue($rs['bulan'], DATATYPE_NUMBER, "DB"));
		$dtlrows = ($rscascade) ? $rscascade->getRows() : [];

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["gaji_tu_sma"]->Row_Deleting($dtlrow);
			if (!$success)
				break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["gaji_tu_sma"]->delete($dtlrow); // Delete
				if (!$success)
					break;
			}
		}
		...
		if ($success) {
			foreach ($dtlrows as $dtlrow)
				$GLOBALS["gaji_tu_sma"]->Row_Deleted($dtlrow);
		}
		if ($success)
	...
	{
		global $Language;
		if ($pageName == "m_tu_smaview.php")
			return $Language->phrase("View");
		elseif ($pageName == "m_tu_smaedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_tu_smaadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_tu_smaview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_tu_smaview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_tu_smaadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_tu_smaadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_tu_smaedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_tu_smaedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_tu_smaadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_tu_smaadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_tu_smadelete.php", $this->getUrlParm());
	}
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		$querymy = "INSERT INTO m_sma VALUES (NULL,'".$tahun."','".$row["bulan"]."',NULL,NULL)";
	...
	//die;
		...
		//print_r($tunjangan_jabatan);
		//die;
		//perhitungan

		$tambahan = ($piket["value"] * $absen["piket"]) +  ($lembur * $absen["lembur"]);
		$inval2 = ($inval * $absen["inval"]) ;
		$sub_total = $tunjangan_jabatan['value'] + ($gaji_pokok["value"] * $query["jjm"]) + ($kehadiran["value"]* $query["jjm"]) +$tunjangan_berkala["value"] + $tunjanagan_tambahan["value"] + ($absen["lembur"] * $query2["lembur"]) + ($absen["piket"] * $piket["value"]) + $inval2 + $query2["tunjangan_jabatan"] + $query2["jam_lebih"] + $query2["reward"]; 
		$komponen_gapok= ($gaji_pokok["value"] * $query["jjm"]);
		$value_kehadiran = ($kehadiran["value"]* $query["jjm"]);
		$total = ($sub_total + $tambahan) - $penyesuaian;
		$myquery = "INSERT INTO gaji_sma VALUES (NULL,'".$query["nip"]."','".date('Y-m-d H:i:s')."','".date('Y-m-d')."',NULL,NULL,'".$query["jabatan"]."',NULL,'".$total."',NULL,NULL,NULL,NULL,NULL,NULL,NULL,'".$sub_total."','".$penyesuaian."','4','".$tambahan."',NULL,'".$query['jjm']."',NULL,NULL,NULL,'".$value_kehadiran."',NULL,NULL,'".$komponen_gapok."',NULL, NULL, '".$tahun."','".$row["bulan"]."', NULL,NULL)";
	...
	function Row_Deleting(&$rs) {
		...
		$bulan = $rs["bulan"];
		$id = $rs["id"];
		$delete = ("DELETE FROM generate_pertahun WHERE tahun ='".$tahun."' AND bulan ='".$bulan."' AND profesi ='2'");
		$delete_detil = ("DELETE FROM gaji_tu_sma WHERE pid='".$id."'");
		return TRUE;
	}
