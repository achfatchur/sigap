...
/**
 * Table class for m_bpjs
 */
class m_bpjs extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_bpjs';
		$this->TableName = 'm_bpjs';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_bpjs`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// jenjang
		$this->jenjang = new DbField('m_bpjs', 'm_bpjs', 'x_jenjang', 'jenjang', '`jenjang`', '`jenjang`', 3, 11, -1, FALSE, '`jenjang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// golongan
		$this->golongan = new DbField('m_bpjs', 'm_bpjs', 'x_golongan', 'golongan', '`golongan`', '`golongan`', 200, 50, -1, FALSE, '`golongan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->golongan->Sortable = TRUE; // Allow sort
		$this->fields['golongan'] = &$this->golongan;

		// value
		$this->value = new DbField('m_bpjs', 'm_bpjs', 'x_value', 'value', '`value`', '`value`', 20, 20, -1, FALSE, '`value`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_bpjs`";
	}
	...
	{
		global $Language;
		if ($pageName == "m_bpjsview.php")
			return $Language->phrase("View");
		elseif ($pageName == "m_bpjsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_bpjsadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_bpjsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_bpjsview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_bpjsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_bpjsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_bpjsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_bpjsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_bpjsdelete.php", $this->getUrlParm());
	}
