...
/**
 * Table class for v_gaji_smp
 */
class v_gaji_smp extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'v_gaji_smp';
		$this->TableName = 'v_gaji_smp';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`v_gaji_smp`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// pegawai
		$this->pegawai = new DbField('v_gaji_smp', 'v_gaji_smp', 'x_pegawai', 'pegawai', '`pegawai`', '`pegawai`', 200, 50, -1, FALSE, '`pegawai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// rekbank
		$this->rekbank = new DbField('v_gaji_smp', 'v_gaji_smp', 'x_rekbank', 'rekbank', '`rekbank`', '`rekbank`', 200, 255, -1, FALSE, '`rekbank`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rekbank->Sortable = TRUE; // Allow sort
		$this->fields['rekbank'] = &$this->rekbank;

		// total
		$this->total = new DbField('v_gaji_smp', 'v_gaji_smp', 'x_total', 'total', '`total`', '`total`', 20, 19, -1, FALSE, '`total`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->total->Sortable = TRUE; // Allow sort
		$this->total->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['total'] = &$this->total;

		// potongan
		$this->potongan = new DbField('v_gaji_smp', 'v_gaji_smp', 'x_potongan', 'potongan', '`potongan`', '`potongan`', 20, 19, -1, FALSE, '`potongan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// pid
		$this->pid = new DbField('v_gaji_smp', 'v_gaji_smp', 'x_pid', 'pid', '`pid`', '`pid`', 3, 11, -1, FALSE, '`pid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getMasterFilter()
	{
		...
		// Master filter
		$masterFilter = "";
		if ($this->getCurrentMasterTable() == "v_m_smp") {
	...
	// Master filter
	public function sqlMasterFilter_v_m_smp()
	{
		return "`id`=@id@";
	}

	// Detail filter
	public function sqlDetailFilter_v_m_smp()
	{
		return "`pid`=@pid@";
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`v_gaji_smp`";
	}
	...
	{
		global $Language;
		if ($pageName == "v_gaji_smpview.php")
			return $Language->phrase("View");
		elseif ($pageName == "v_gaji_smpedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "v_gaji_smpadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("v_gaji_smpview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("v_gaji_smpview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "v_gaji_smpadd.php?" . $this->getUrlParm($parm);
		else
			$url = "v_gaji_smpadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v_gaji_smpedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v_gaji_smpadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("v_gaji_smpdelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "v_m_smp" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
