...
/**
 * Table class for m_lembur
 */
class m_lembur extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_lembur';
		$this->TableName = 'm_lembur';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_lembur`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('m_lembur', 'm_lembur', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// jenjang_id
		$this->jenjang_id = new DbField('m_lembur', 'm_lembur', 'x_jenjang_id', 'jenjang_id', '`jenjang_id`', '`jenjang_id`', 3, 11, -1, FALSE, '`jenjang_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// jabatan_id
		$this->jabatan_id = new DbField('m_lembur', 'm_lembur', 'x_jabatan_id', 'jabatan_id', '`jabatan_id`', '`jabatan_id`', 3, 11, -1, FALSE, '`jabatan_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// value_perjam
		$this->value_perjam = new DbField('m_lembur', 'm_lembur', 'x_value_perjam', 'value_perjam', '`value_perjam`', '`value_perjam`', 20, 20, -1, FALSE, '`value_perjam`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->value_perjam->Sortable = TRUE; // Allow sort
		$this->value_perjam->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['value_perjam'] = &$this->value_perjam;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_lembur`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->value_perjam->DbValue = $row['value_perjam'];
	}
	...
	{
		global $Language;
		if ($pageName == "m_lemburview.php")
			return $Language->phrase("View");
		elseif ($pageName == "m_lemburedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_lemburadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_lemburview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_lemburview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_lemburadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_lemburadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_lemburedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_lemburadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_lemburdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->value_perjam->setDbValue($rs->fields('value_perjam'));
	}
	...
	public function renderEditRow()
	{
		...
		// value_perjam
		$this->value_perjam->EditAttrs["class"] = "form-control";
		$this->value_perjam->EditCustomAttributes = "";
		$this->value_perjam->EditValue = $this->value_perjam->CurrentValue;
		$this->value_perjam->PlaceHolder = RemoveHtml($this->value_perjam->caption());
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->value_perjam);
				} else {
					$doc->exportCaption($this->id);
					$doc->exportCaption($this->jenjang_id);
					$doc->exportCaption($this->jabatan_id);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->value_perjam);
					} else {
						$doc->exportField($this->id);
						$doc->exportField($this->jenjang_id);
						$doc->exportField($this->jabatan_id);
