...
/**
 * Table class for m_sakit
 */
class m_sakit extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_sakit';
		$this->TableName = 'm_sakit';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_sakit`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('m_sakit', 'm_sakit', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// jenjang_id
		$this->jenjang_id = new DbField('m_sakit', 'm_sakit', 'x_jenjang_id', 'jenjang_id', '`jenjang_id`', '`jenjang_id`', 3, 11, -1, FALSE, '`jenjang_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// jabatan
		$this->jabatan = new DbField('m_sakit', 'm_sakit', 'x_jabatan', 'jabatan', '`jabatan`', '`jabatan`', 3, 11, -1, FALSE, '`jabatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// sertif
		$this->sertif = new DbField('m_sakit', 'm_sakit', 'x_sertif', 'sertif', '`sertif`', '`sertif`', 3, 11, -1, FALSE, '`sertif`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// perhari
		$this->perhari = new DbField('m_sakit', 'm_sakit', 'x_perhari', 'perhari', '`perhari`', '`perhari`', 20, 20, -1, FALSE, '`perhari`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->perhari->Sortable = TRUE; // Allow sort
		$this->perhari->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['perhari'] = &$this->perhari;

		// perjam
		$this->perjam = new DbField('m_sakit', 'm_sakit', 'x_perjam', 'perjam', '`perjam`', '`perjam`', 20, 20, -1, FALSE, '`perjam`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->perjam->Sortable = TRUE; // Allow sort
		$this->perjam->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['perjam'] = &$this->perjam;

		// type
		$this->type = new DbField('m_sakit', 'm_sakit', 'x_type', 'type', '`type`', '`type`', 3, 11, -1, FALSE, '`type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_sakit`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->perhari->DbValue = $row['perhari'];
		$this->perjam->DbValue = $row['perjam'];
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "m_sakitedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_sakitadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_sakitview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_sakitview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_sakitadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_sakitadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_sakitedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_sakitadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_sakitdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->perhari->setDbValue($rs->fields('perhari'));
		$this->perjam->setDbValue($rs->fields('perjam'));
	...
	public function renderEditRow()
	{
		...
		$this->perhari->EditValue = $this->perhari->CurrentValue;
		...
		$this->perjam->EditValue = $this->perjam->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->perhari);
					$doc->exportCaption($this->perjam);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->perhari);
						$doc->exportField($this->perjam);
