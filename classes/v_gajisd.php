...
/**
 * Table class for v_gajisd
 */
class v_gajisd extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'v_gajisd';
		$this->TableName = 'v_gajisd';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`v_gajisd`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// pegawai
		$this->pegawai = new DbField('v_gajisd', 'v_gajisd', 'x_pegawai', 'pegawai', '`pegawai`', '`pegawai`', 200, 50, -1, FALSE, '`pegawai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// rekbank
		$this->rekbank = new DbField('v_gajisd', 'v_gajisd', 'x_rekbank', 'rekbank', '`rekbank`', '`rekbank`', 200, 255, -1, FALSE, '`rekbank`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rekbank->Sortable = TRUE; // Allow sort
		$this->fields['rekbank'] = &$this->rekbank;

		// total
		$this->total = new DbField('v_gajisd', 'v_gajisd', 'x_total', 'total', '`total`', '`total`', 20, 20, -1, FALSE, '`total`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->total->Sortable = TRUE; // Allow sort
		$this->total->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['total'] = &$this->total;

		// potongan
		$this->potongan = new DbField('v_gajisd', 'v_gajisd', 'x_potongan', 'potongan', '`potongan`', '`potongan`', 20, 20, -1, FALSE, '`potongan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// pid
		$this->pid = new DbField('v_gajisd', 'v_gajisd', 'x_pid', 'pid', '`pid`', '`pid`', 3, 11, -1, FALSE, '`pid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getMasterFilter()
	{
		...
		// Master filter
		$masterFilter = "";
		if ($this->getCurrentMasterTable() == "v_m_sd") {
	...
	// Master filter
	public function sqlMasterFilter_v_m_sd()
	{
		return "`id`=@id@";
	}

	// Detail filter
	public function sqlDetailFilter_v_m_sd()
	{
		return "`pid`=@pid@";
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`v_gajisd`";
	}
	...
	{
		global $Language;
		if ($pageName == "v_gajisdview.php")
			return $Language->phrase("View");
		elseif ($pageName == "v_gajisdedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "v_gajisdadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("v_gajisdview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("v_gajisdview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "v_gajisdadd.php?" . $this->getUrlParm($parm);
		else
			$url = "v_gajisdadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v_gajisdedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v_gajisdadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("v_gajisddelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "v_m_sd" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
