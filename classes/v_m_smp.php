...
/**
 * Table class for v_m_smp
 */
class v_m_smp extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'v_m_smp';
		$this->TableName = 'v_m_smp';
		$this->TableType = 'VIEW';
		...
		// id
		$this->id = new DbField('v_m_smp', 'v_m_smp', 'x_id', 'id', '`id`', '`id`', 3, 10, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// datetime
		$this->datetime = new DbField('v_m_smp', 'v_m_smp', 'x_datetime', 'datetime', '`datetime`', CastDateFieldForLike("`datetime`", 0, "DB"), 135, 19, 0, FALSE, '`datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->datetime->Sortable = TRUE; // Allow sort
		$this->datetime->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['datetime'] = &$this->datetime;

		// createby
		$this->createby = new DbField('v_m_smp', 'v_m_smp', 'x_createby', 'createby', '`createby`', '`createby`', 3, 10, -1, FALSE, '`createby`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tahun
		$this->tahun = new DbField('v_m_smp', 'v_m_smp', 'x_tahun', 'tahun', '`tahun`', '`tahun`', 2, 5, -1, FALSE, '`tahun`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// bulan
		$this->bulan = new DbField('v_m_smp', 'v_m_smp', 'x_bulan', 'bulan', '`bulan`', '`bulan`', 3, 10, -1, FALSE, '`bulan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "v_gaji_smp") {
			$detailUrl = $GLOBALS["v_gaji_smp"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "v_m_smplist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`v_m_smp`";
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		// Cascade Update detail table 'v_gaji_smp'
		$cascadeUpdate = FALSE;
		$rscascade = [];
		...
		if ($cascadeUpdate) {
			if (!isset($GLOBALS["v_gaji_smp"]))
				$GLOBALS["v_gaji_smp"] = new v_gaji_smp();
			$rswrk = $GLOBALS["v_gaji_smp"]->loadRs("`pid` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB'));
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["v_gaji_smp"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["v_gaji_smp"]->update($rscascade, $rskey, $rswrk->fields);
				if (!$success)
					return FALSE;

				// Call Row_Updated event
				$GLOBALS["v_gaji_smp"]->Row_Updated($rsdtlold, $rsdtlnew);
				$rswrk->moveNext();
			}
		}
	...
	{
		global $Language;
		if ($pageName == "v_m_smpview.php")
			return $Language->phrase("View");
		elseif ($pageName == "v_m_smpedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "v_m_smpadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("v_m_smpview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("v_m_smpview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "v_m_smpadd.php?" . $this->getUrlParm($parm);
		else
			$url = "v_m_smpadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("v_m_smpedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("v_m_smpedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("v_m_smpadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("v_m_smpadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("v_m_smpdelete.php", $this->getUrlParm());
	}
