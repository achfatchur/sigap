...
/**
 * Table class for generate_pertahun_smp
 */
class generate_pertahun_smp extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'generate_pertahun_smp';
		$this->TableName = 'generate_pertahun_smp';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`generate_pertahun_smp`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('generate_pertahun_smp', 'generate_pertahun_smp', 'x_id', 'id', '`id`', '`id`', 3, 10, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// profesi
		$this->profesi = new DbField('generate_pertahun_smp', 'generate_pertahun_smp', 'x_profesi', 'profesi', '`profesi`', '`profesi`', 3, 10, -1, FALSE, '`profesi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// tahun
		$this->tahun = new DbField('generate_pertahun_smp', 'generate_pertahun_smp', 'x_tahun', 'tahun', '`tahun`', '`tahun`', 3, 10, -1, FALSE, '`tahun`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// bulan
		$this->bulan = new DbField('generate_pertahun_smp', 'generate_pertahun_smp', 'x_bulan', 'bulan', '`bulan`', '`bulan`', 200, 50, -1, FALSE, '`bulan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->bulan->Sortable = TRUE; // Allow sort
		$this->bulan->Lookup = new Lookup('bulan', 'bulan', FALSE, 'id', ["bulan","","",""], [], [], [], [], [], [], '', '');
		$this->fields['bulan'] = &$this->bulan;

		// bulan2
		$this->bulan2 = new DbField('generate_pertahun_smp', 'generate_pertahun_smp', 'x_bulan2', 'bulan2', '`bulan2`', '`bulan2`', 3, 10, -1, FALSE, '`bulan2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`generate_pertahun_smp`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "generate_pertahun_smplist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "generate_pertahun_smpview.php")
			return $Language->phrase("View");
		elseif ($pageName == "generate_pertahun_smpedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "generate_pertahun_smpadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("generate_pertahun_smpview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("generate_pertahun_smpview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "generate_pertahun_smpadd.php?" . $this->getUrlParm($parm);
		else
			$url = "generate_pertahun_smpadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("generate_pertahun_smpedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("generate_pertahun_smpadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("generate_pertahun_smpdelete.php", $this->getUrlParm());
	}
...
//$profesi =$rsnew['profesi'];
			...
			$reward = ExecuteScalar("select value from m_reward where jenjang ='".$query["jenjang_id"]."' AND jabatan ='".$query["jabatan"]."'");
					...
					$myquery2 = "INSERT INTO gaji_karyawan_smp VALUES (NULL, '".$query["nip"]."','".$query["jabatan"]."','3','".$komponen_gapok."',NULL,'".$inval2."','".$c_jjm."','".$sub_total."','".$penyesuaian."','".$tambahan."','".$total."','".$pid."','".$v_kehadiran."',NULL,'".$tahun."', '".$bulan."','".$v_voucher."',NULL,'".$value_pensiun."','".$value_hari_tua."','".$pph21."','".$solve_bpjs."','".$solved_npwp."')";
