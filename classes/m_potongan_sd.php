...
/**
 * Table class for m_potongan_sd
 */
class m_potongan_sd extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_potongan_sd';
		$this->TableName = 'm_potongan_sd';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_potongan_sd`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('m_potongan_sd', 'm_potongan_sd', 'x_id', 'id', '`id`', '`id`', 3, 10, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// tahun
		$this->tahun = new DbField('m_potongan_sd', 'm_potongan_sd', 'x_tahun', 'tahun', '`tahun`', '`tahun`', 3, 10, -1, FALSE, '`tahun`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// bulan
		$this->bulan = new DbField('m_potongan_sd', 'm_potongan_sd', 'x_bulan', 'bulan', '`bulan`', '`bulan`', 3, 10, -1, FALSE, '`bulan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// c_by
		$this->c_by = new DbField('m_potongan_sd', 'm_potongan_sd', 'x_c_by', 'c_by', '`c_by`', '`c_by`', 3, 10, -1, FALSE, '`c_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// datetime
		$this->datetime = new DbField('m_potongan_sd', 'm_potongan_sd', 'x_datetime', 'datetime', '`datetime`', CastDateFieldForLike("`datetime`", 0, "DB"), 135, 19, 0, FALSE, '`datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->datetime->Sortable = TRUE; // Allow sort
		$this->datetime->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['datetime'] = &$this->datetime;

		// import_file
		$this->import_file = new DbField('m_potongan_sd', 'm_potongan_sd', 'x_import_file', 'import_file', '`import_file`', '`import_file`', 201, 65535, -1, TRUE, '`import_file`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "potongan_sd") {
			$detailUrl = $GLOBALS["potongan_sd"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
			$detailUrl .= "&fk_tahun=" . urlencode($this->tahun->CurrentValue);
			$detailUrl .= "&fk_bulan=" . urlencode($this->bulan->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "m_potongan_sdlist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_potongan_sd`";
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		// Cascade Update detail table 'potongan_sd'
		$cascadeUpdate = FALSE;
		$rscascade = [];
		...
		if ($cascadeUpdate) {
			...
			$rswrk = $GLOBALS["potongan_sd"]->loadRs("`pid` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB') . " AND " . "`tahun` = " . QuotedValue($rsold['tahun'], DATATYPE_NUMBER, 'DB') . " AND " . "`bulan` = " . QuotedValue($rsold['bulan'], DATATYPE_NUMBER, 'DB'));
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["potongan_sd"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["potongan_sd"]->update($rscascade, $rskey, $rswrk->fields);
				if (!$success)
					return FALSE;

				// Call Row_Updated event
				$GLOBALS["potongan_sd"]->Row_Updated($rsdtlold, $rsdtlnew);
				$rswrk->moveNext();
			}
		}
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		// Cascade delete detail table 'potongan_sd'
		if (!isset($GLOBALS["potongan_sd"]))
			$GLOBALS["potongan_sd"] = new potongan_sd();
		$rscascade = $GLOBALS["potongan_sd"]->loadRs("`pid` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB") . " AND " . "`tahun` = " . QuotedValue($rs['tahun'], DATATYPE_NUMBER, "DB") . " AND " . "`bulan` = " . QuotedValue($rs['bulan'], DATATYPE_NUMBER, "DB"));
		$dtlrows = ($rscascade) ? $rscascade->getRows() : [];

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["potongan_sd"]->Row_Deleting($dtlrow);
			if (!$success)
				break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["potongan_sd"]->delete($dtlrow); // Delete
				if (!$success)
					break;
			}
		}
		...
		if ($success) {
			foreach ($dtlrows as $dtlrow)
				$GLOBALS["potongan_sd"]->Row_Deleted($dtlrow);
		}
		if ($success)
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "m_potongan_sdedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_potongan_sdadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_potongan_sdview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_potongan_sdview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_potongan_sdadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_potongan_sdadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_potongan_sdedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_potongan_sdedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_potongan_sdadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_potongan_sdadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_potongan_sddelete.php", $this->getUrlParm());
	}
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		if($reader) {
			...
			foreach($records as $row) {
				...
				if ($XLSXdata != "") {
					$myquery = "INSERT INTO `potongan_sd` VALUES ".rtrim($XLSXdata, ',') . ';';
