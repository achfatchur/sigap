	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//echo "Row Inserted"
			...
			foreach ($data_bulan as $bulan) {
				...
				$querymy2 = "INSERT INTO generate_pertahun_sd VALUES(NULL,'".$tahun."',NULL,'".$profesi."','".$bulan."')";
				...
				$delete_clone ="delete FROM generate_pertahun_sd WHERE id < '".$id."' AND tahun ='".$tahun."' AND bulan2='".$bulan."' AND profesi ='".$profesi."'";	
				...
				if($this->profesi->CurrentValue == '1'){
					...
					$row2 = ExecuteRows("SELECT * FROM pegawai where jenjang_id ='2' AND `type`= '1'");
		...
		$clone_delete = execute($delete_m_sd);
						...
						$row2 = ExecuteRows("SELECT * FROM pegawai where jenjang_id ='2' AND `type`= '2'");
				...
				foreach($row2 as $query){
					...
					$row2 = ExecuteRows("SELECT * FROM pegawai where jenjang_id ='2' AND `type`= '3'");	
				foreach($row2 as $query){
