...
/**
 * Table class for generate_pertahun_sd
 */
class generate_pertahun_sd extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'generate_pertahun_sd';
		$this->TableName = 'generate_pertahun_sd';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`generate_pertahun_sd`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('generate_pertahun_sd', 'generate_pertahun_sd', 'x_id', 'id', '`id`', '`id`', 3, 10, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// profesi
		$this->profesi = new DbField('generate_pertahun_sd', 'generate_pertahun_sd', 'x_profesi', 'profesi', '`profesi`', '`profesi`', 3, 10, -1, FALSE, '`profesi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// tahun
		$this->tahun = new DbField('generate_pertahun_sd', 'generate_pertahun_sd', 'x_tahun', 'tahun', '`tahun`', '`tahun`', 3, 10, -1, FALSE, '`tahun`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// bulan
		$this->bulan = new DbField('generate_pertahun_sd', 'generate_pertahun_sd', 'x_bulan', 'bulan', '`bulan`', '`bulan`', 200, 50, -1, FALSE, '`bulan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->bulan->Sortable = TRUE; // Allow sort
		$this->bulan->Lookup = new Lookup('bulan', 'bulan', FALSE, 'id', ["bulan","","",""], [], [], [], [], [], [], '', '');
		$this->fields['bulan'] = &$this->bulan;

		// bulan2
		$this->bulan2 = new DbField('generate_pertahun_sd', 'generate_pertahun_sd', 'x_bulan2', 'bulan2', '`bulan2`', '`bulan2`', 3, 10, -1, FALSE, '`bulan2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`generate_pertahun_sd`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "generate_pertahun_sdlist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "generate_pertahun_sdview.php")
			return $Language->phrase("View");
		elseif ($pageName == "generate_pertahun_sdedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "generate_pertahun_sdadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("generate_pertahun_sdview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("generate_pertahun_sdview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "generate_pertahun_sdadd.php?" . $this->getUrlParm($parm);
		else
			$url = "generate_pertahun_sdadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("generate_pertahun_sdedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("generate_pertahun_sdadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("generate_pertahun_sddelete.php", $this->getUrlParm());
	}
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//	foreach ($bulan_query as $row) {
				...
				foreach ($data_bulan as $bulan) {
					...
					$querymy2 = "INSERT INTO generate_pertahun_sd VALUES(NULL,'".$tahun."',NULL,NULL,'".$bulan."')";
					$Result2 = Execute($querymy2);

					//delete field bulan ketika generate bulan2
					$delete ="DELETE FROM generate_pertahun_sd WHERE bulan2 is null";
					$delete2 = Execute($delete);

					//$limit = ExecuteRow("select count(*) from generate_pertahun where tahun ='".$tahun."' AND bulan2='".$bulan."' AND profesi ='".$profesi."'");
					//print_r($limit);
					//die;

					$delete_clone ="delete FROM generate_pertahun_sd WHERE id < '".$id."' AND tahun ='".$tahun."' AND bulan2='".$bulan."'";	
					...
					//if($this->profesi->CurrentValue == '1'){
						$delete_detil_karyawan ="DELETE FROM gaji WHERE tahun ='".$tahun."' AND bulan='".$bulan."'";	
						$detil_delete = execute($delete_detil_karyawan);			
						$delete_m_sd ="DELETE FROM m_sd WHERE tahun ='".$tahun."' AND bulan='".$bulan."' ORDER BY id asc";	
						$clone_delete = execute($delete_m_sd); 

						//$querymy2 = "INSERT INTO m_sma VALUES (NULL,'".$tahun."',NULL,'".$bulan."')";
						//$Result2 = Execute($querymy2);	

						$querymy = "INSERT INTO m_sd VALUES (NULL,'".$tahun."','".$bulan."',NULL,NULL)";
						$Result = Execute($querymy);

						// print_r($querymy);
						// die;

						$pid = ExecuteScalar("select id from m_sd order by id DESC limit 1");
						$row2_guru = ExecuteRows("SELECT * FROM pegawai where jenjang_id ='2' AND `type`= '1'");

						//print_r($row);
						//die;

					foreach($row2_guru as $query){

						//value piket
						$reward = ExecuteScalar("select value from m_reward where jenjang ='".$query["jenjang_id"]."'");
						$piket= ExecuteRow("select * from m_piket where jenjang='2' AND jenis_sertif ='".$query['sertif']."' AND type_jabatan='1'");

						//value kehadiran
						$kehadiran=ExecuteRow("Select * from m_kehadiran where jenjang='2' AND jenis_jabatan='1' AND sertif='".$query['sertif']."'");	

						//print_r($kehadiran);
						//die;																		

						$tunjanagan_tambahan=ExecuteRow("select * from tunjangan_tambahan where jenjang='2' AND kualifikasi='".$query['tambahan']."'");
				...
				//if($query['jabatan'] == '5'){
					$tunjangan_berkala=ExecuteRow("SELECT * FROM tunjangan_berkala WHERE jenjang='".$query['jenjang_id']."' AND kualifikasi ='".$query['jabatan']."' AND lama='".$query['periode_jabatan']."'");
				...
				//}
						...
						$gaji_pokok=ExecuteRow("SELECT * FROM gaji_pokok WHERE jenjang_id='2' AND lama_kerja='".$query['lama_kerja']."'");
						$tunjangan_jabatan=ExecuteRow("SELECT * FROM tunjangan_jabatan WHERE unit='2' AND jabatan='".$query['jabatan']."'");
				...
				$penyesuaian = $solve_bpjs + $value_pensiun + $value_hari_tua + $pph21 + ($absen_tidak * $absen["absen"]) + ($absen_tidak * $absen["izin"]) + ($sakit * $absen["sakit"]) + ($telambat * $absen["terlambat"]) + ($pulang_cepat * $absen['pulang_cepat']);
						...
						//	die;
							...
							//print_r($solved);
							//die;
							$solve_jabatan = 1 * $query["jabatan"];
					$myquery = "INSERT INTO gaji VALUES (NULL,'".$query["nip"]."','".date('Y-m-d H:i:s')."','".date('Y-m-d')."','".$c_lembur."','".$v_lembur."','".$solve_jabatan."','".$gaji_pokok["value"]."','".$total."','".$reward."','".$inval2."','".$piket_new."','".$v_piket."','".$solved."','".$tj_jbtn."','".$v_jjm."','".$sub_total."','".$penyesuaian."','".$query["jenjang_id"]."','".$tambahan."','".$pid."','".$v_jjm."','".$query["type"]."','".$sertif."','".$tgs_tmbhn."','".$v_kehadiran."','".$solve_periode."','".$solve_value_per."','".$komponen_gapok."','".$lm_kerja."', NULL, '".$tahun."','".$bulan."', NULL,'".$v_voucher."','".$value_pensiun."','".$value_hari_tua."','".$pph21."','".$solved_npwp."','".$solve_bpjs."')";
					...
					//	die;
						...
						$delete_all ="DELETE FROM solved_sd WHERE tahun ='".$tahun."' AND bulan='".$bulan."' AND nip='".$query["nip"]."'";	
						...
						$clone_all = execute($delete_all_unit);
				...
				//}elseif($this->profesi->CurrentValue == '2'){
							...
							$detil_delete = execute($delete_detil_karyawan);			
							$delete_m_sd ="DELETE FROM m_tu_sd WHERE tahun ='".$tahun."' AND bulan='".$bulan."' ORDER BY id asc";	
							$clone_delete = execute($delete_m_sd);
							$querymy = "INSERT INTO m_tu_sd VALUES (NULL,'".$tahun."','".$bulan."',NULL,NULL)";
							$Result = Execute($querymy);
							$pid = ExecuteScalar("select id from m_tu_sd order by id DESC limit 1");
							$row2_tu = ExecuteRows("SELECT * FROM pegawai where jenjang_id ='2' AND `type`= '2'");

												//print_r($row);
												//die;

					foreach($row2_tu as $query){
						$reward = ExecuteScalar("select value from m_reward where jenjang ='".$query["jenjang_id"]."'");

						//$kehadiran=ExecuteRow("Select * from m_kehadiran where jenjang='2' AND jenis_jabatan='1'");	
						$piket= ExecuteRow("select * from m_piket where jenjang='2' AND type_jabatan='2'");	
						$kehadiran=ExecuteRow("Select * from m_kehadiran where jenjang='2' AND jenis_jabatan='2'");																		
						$tunjanagan_tambahan=ExecuteRow("select * from tunjangan_tambahan where jenjang='2' AND kualifikasi='".$query['tambahan']."'");
						$absen =ExecuteRow("SELECT * FROM penyesuaian INNER JOIN m_penyesuaian ON m_penyesuaian.id = penyesuaian.m_id WHERE penyesuaian.nip='".$query['nip']."' AND  m_penyesuaian.tahun='".$tahun."' AND m_penyesuaian.bulan='".$bulan."' ORDER BY m_penyesuaian.id DESC");	
						...
						$lembur = ExecuteScalar("SELECT value_perjam FROM m_lembur WHERE jenjang_id='".$query["jenjang_id"]."'");
						$gaji_pokok=ExecuteRow("SELECT * FROM gaji_pokok_tu WHERE jenjang_id='2' AND lama_kerja='".$query['lama_kerja']."' AND ijazah= '".$query['pendidikan']."'");

							//print_r($gaji_pokok);
							//die;

						$tunjangan_jabatan=ExecuteRow("SELECT * FROM tunjangan_jabatan WHERE unit='2' AND jabatan='".$query['jabatan']."'");		
						$tunjanagan_khusus=ExecuteScalar("select value from tunjangan_khusu where unit='2' AND jabatan ='".$query["type"]."' ");
						...
						//die;	
						$solve_jabatan = 1 * $query["jabatan"];
						$myquery2 = "INSERT INTO gaji_tu_sd VALUES (NULL,NULL,'".$query["nip"]."','".$query["jenjang_id"]."','".$solve_jabatan."',NULL,'".$komponen_gapok."','".$hadir	."','".$c_lembur."','".$lembur."','".$reward."','".$inval2."','".$c_piket."','".$piket["value"]."','".$solved."','".$tj_jbtn."','".$penyesuaian."','".$sub_total."','".$tambahan_value."','".$total."','".$pid."','".$khusus."','".$tambahan."','2','".$pendidikan."','".$lm_kerja."','".$sertif."','".$kehadiran["value"]."','".$tahun."','".$bulan."','".$v_voucher."',NULL,NULL,'".$value_pensiun."','".$value_hari_tua."','".$pph21."','".$solve_bpjs."','".$solved_npwp."')";
					...
					//}else{
					$delete_detil_karyawan ="DELETE FROM gaji_karyawan_sd WHERE tahun ='".$tahun."' AND bulan='".$bulan."'";	
					$detil_delete = execute($delete_detil_karyawan);			
					$delete_m_sd ="DELETE FROM m_karyawan_sd WHERE tahun ='".$tahun."' AND bulan='".$bulan."' ORDER BY id asc";	
					$clone_delete = execute($delete_m_sd);
					$querymy = "INSERT INTO m_karyawan_sd VALUES (NULL,'".$tahun."','".$bulan."',NULL,NULL)";
					$Result = Execute($querymy);
					$pid = ExecuteScalar("select id from m_karyawan_sd order by id DESC limit 1");
					$row2_karyawan = ExecuteRows("SELECT * FROM pegawai where jenjang_id ='2' AND `type`= '3'");	
					foreach($row2_karyawan as $query){
					...
					$kehadiran= ExecuteRow("Select * from m_kehadiran where jenjang='2' AND jabatan= '".$query["jabatan"]."'");	

						//GAJI POKOK
					$query2 = ExecuteRow("SELECT * FROM jabatan JOIN gajitunjangan ON jabatan.id = gajitunjangan.pidjabatan where gajitunjangan.pidjabatan='9'");
					$querypokok = ExecuteRow("SELECT * FROM gaji_pokok_kayawan where jenjang_id='2' AND jabatan_id= '".$query["jabatan"]."'");		
					$inval = ExecuteScalar("SELECT value FROM m_inval WHERE jenjang_id='".$query["jenjang_id"]."'");
					$lembur = ExecuteScalar("SELECT value_perjam FROM m_lembur WHERE jenjang_id='".$query["jenjang_id"]."'");							
					$piket= ExecuteRow("select * from m_piket where jenjang='2' AND type_jabatan='3'");							//Gaji Pokok	
					$kehadiran = ExecuteRow("select * from m_kehadiran where jenjang ='2' AND jenis_jabatan='3' AND jabatan='".$query["jabatan"]."'");
					$gaji_pokok=ExecuteRow("SELECT * FROM gaji_pokok_kayawan WHERE jenjang_id='2' AND jabatan_id= '".$query["jabatan"]."'");	
					$absen =ExecuteRow("SELECT * FROM penyesuaian INNER JOIN m_penyesuaian ON m_penyesuaian.id = penyesuaian.m_id WHERE penyesuaian.nip='".$query['nip']."' AND  m_penyesuaian.tahun='".$tahun."' AND m_penyesuaian.bulan='".$bulan."' ORDER BY m_penyesuaian.id DESC");	
					...
					//die;
					$solve_jabatan = 1 * $query["jabatan"];
					$myquery2 = "INSERT INTO gaji_karyawan_sd VALUES (NULL, '".$query["nip"]."','".$solve_jabatan."','4','".$komponen_gapok."',NULL,'".$inval2."','".$c_jjm."','".$sub_total."','".$penyesuaian."','".$tambahan."','".$total."','".$pid."','".$v_kehadiran."',NULL,'".$tahun."', '".$bulan."','".$v_voucher."',NULL,'".$value_pensiun."','".$value_hari_tua."','".$solve_bpjs."','".$solved_npwp."','".$pph21."')";
