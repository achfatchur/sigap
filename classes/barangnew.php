...
/**
 * Table class for barangnew
 */
class barangnew extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'barangnew';
		$this->TableName = 'barangnew';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`barangnew`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// kode_barang
		$this->kode_barang = new DbField('barangnew', 'barangnew', 'x_kode_barang', 'kode_barang', '`kode_barang`', '`kode_barang`', 200, 50, -1, FALSE, '`kode_barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kode_barang->Sortable = TRUE; // Allow sort
		$this->fields['kode_barang'] = &$this->kode_barang;

		// nama_barang
		$this->nama_barang = new DbField('barangnew', 'barangnew', 'x_nama_barang', 'nama_barang', '`nama_barang`', '`nama_barang`', 200, 50, -1, FALSE, '`nama_barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`barangnew`";
	}
	...
	{
		global $Language;
		if ($pageName == "barangnewview.php")
			return $Language->phrase("View");
		elseif ($pageName == "barangnewedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "barangnewadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("barangnewview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("barangnewview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "barangnewadd.php?" . $this->getUrlParm($parm);
		else
			$url = "barangnewadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("barangnewedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("barangnewadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("barangnewdelete.php", $this->getUrlParm());
	}
