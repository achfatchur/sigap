...
/**
 * Table class for v_totalgajikaryawansmp
 */
class v_totalgajikaryawansmp extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'v_totalgajikaryawansmp';
		$this->TableName = 'v_totalgajikaryawansmp';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`v_totalgajikaryawansmp`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('v_totalgajikaryawansmp', 'v_totalgajikaryawansmp', 'x_id', 'id', '`id`', '`id`', 3, 10, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// datetime
		$this->datetime = new DbField('v_totalgajikaryawansmp', 'v_totalgajikaryawansmp', 'x_datetime', 'datetime', '`datetime`', CastDateFieldForLike("`datetime`", 0, "DB"), 135, 19, 0, FALSE, '`datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->datetime->Sortable = TRUE; // Allow sort
		$this->datetime->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['datetime'] = &$this->datetime;

		// bulan
		$this->bulan = new DbField('v_totalgajikaryawansmp', 'v_totalgajikaryawansmp', 'x_bulan', 'bulan', '`bulan`', '`bulan`', 3, 10, -1, FALSE, '`bulan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// tahun
		$this->tahun = new DbField('v_totalgajikaryawansmp', 'v_totalgajikaryawansmp', 'x_tahun', 'tahun', '`tahun`', '`tahun`', 2, 5, -1, FALSE, '`tahun`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// pegawai
		$this->pegawai = new DbField('v_totalgajikaryawansmp', 'v_totalgajikaryawansmp', 'x_pegawai', 'pegawai', '`pegawai`', '`pegawai`', 200, 50, -1, FALSE, '`pegawai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// total
		$this->total = new DbField('v_totalgajikaryawansmp', 'v_totalgajikaryawansmp', 'x_total', 'total', '`total`', '`total`', 20, 19, -1, FALSE, '`total`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->total->Sortable = TRUE; // Allow sort
		$this->total->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['total'] = &$this->total;

		// id1
		$this->id1 = new DbField('v_totalgajikaryawansmp', 'v_totalgajikaryawansmp', 'x_id1', 'id1', '`id1`', '`id1`', 3, 10, -1, FALSE, '`id1`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// status
		$this->status = new DbField('v_totalgajikaryawansmp', 'v_totalgajikaryawansmp', 'x_status', 'status', '`status`', '`status`', 3, 11, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`v_totalgajikaryawansmp`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "v_totalgajikaryawansmplist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "v_totalgajikaryawansmpview.php")
			return $Language->phrase("View");
		elseif ($pageName == "v_totalgajikaryawansmpedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "v_totalgajikaryawansmpadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("v_totalgajikaryawansmpview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("v_totalgajikaryawansmpview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "v_totalgajikaryawansmpadd.php?" . $this->getUrlParm($parm);
		else
			$url = "v_totalgajikaryawansmpadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v_totalgajikaryawansmpedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v_totalgajikaryawansmpadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("v_totalgajikaryawansmpdelete.php", $this->getUrlParm());
	}
