...
/**
 * Table class for m_tidakhadir
 */
class m_tidakhadir extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_tidakhadir';
		$this->TableName = 'm_tidakhadir';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_tidakhadir`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('m_tidakhadir', 'm_tidakhadir', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// jenjang_id
		$this->jenjang_id = new DbField('m_tidakhadir', 'm_tidakhadir', 'x_jenjang_id', 'jenjang_id', '`jenjang_id`', '`jenjang_id`', 3, 11, -1, FALSE, '`jenjang_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// jabatan_id
		$this->jabatan_id = new DbField('m_tidakhadir', 'm_tidakhadir', 'x_jabatan_id', 'jabatan_id', '`jabatan_id`', '`jabatan_id`', 3, 11, -1, FALSE, '`jabatan_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// sertif
		$this->sertif = new DbField('m_tidakhadir', 'm_tidakhadir', 'x_sertif', 'sertif', '`sertif`', '`sertif`', 3, 10, -1, FALSE, '`sertif`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// value
		$this->value = new DbField('m_tidakhadir', 'm_tidakhadir', 'x_value', 'value', '`value`', '`value`', 20, 20, -1, FALSE, '`value`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// perjam_value
		$this->perjam_value = new DbField('m_tidakhadir', 'm_tidakhadir', 'x_perjam_value', 'perjam_value', '`perjam_value`', '`perjam_value`', 20, 20, -1, FALSE, '`perjam_value`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->perjam_value->Sortable = TRUE; // Allow sort
		$this->perjam_value->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['perjam_value'] = &$this->perjam_value;

		// type
		$this->type = new DbField('m_tidakhadir', 'm_tidakhadir', 'x_type', 'type', '`type`', '`type`', 3, 11, -1, FALSE, '`type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_tidakhadir`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->perjam_value->DbValue = $row['perjam_value'];
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "m_tidakhadiredit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_tidakhadiradd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_tidakhadirview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_tidakhadirview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_tidakhadiradd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_tidakhadiradd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_tidakhadiredit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_tidakhadiradd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_tidakhadirdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->perjam_value->setDbValue($rs->fields('perjam_value'));
	...
	public function renderEditRow()
	{
		...
		$this->perjam_value->EditValue = $this->perjam_value->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->perjam_value);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->perjam_value);
