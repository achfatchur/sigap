...
/**
 * Table class for m_smp
 */
class m_smp extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_smp';
		$this->TableName = 'm_smp';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_smp`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('m_smp', 'm_smp', 'x_id', 'id', '`id`', '`id`', 3, 10, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// datetime
		$this->datetime = new DbField('m_smp', 'm_smp', 'x_datetime', 'datetime', '`datetime`', CastDateFieldForLike("`datetime`", 0, "DB"), 135, 19, 0, FALSE, '`datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->datetime->Sortable = TRUE; // Allow sort
		$this->datetime->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['datetime'] = &$this->datetime;

		// createby
		$this->createby = new DbField('m_smp', 'm_smp', 'x_createby', 'createby', '`createby`', '`createby`', 3, 10, -1, FALSE, '`createby`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tahun
		$this->tahun = new DbField('m_smp', 'm_smp', 'x_tahun', 'tahun', '`tahun`', '`tahun`', 18, 4, -1, FALSE, '`tahun`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// bulan
		$this->bulan = new DbField('m_smp', 'm_smp', 'x_bulan', 'bulan', '`bulan`', '`bulan`', 3, 10, -1, FALSE, '`bulan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "gaji_smp") {
			$detailUrl = $GLOBALS["gaji_smp"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
			$detailUrl .= "&fk_tahun=" . urlencode($this->tahun->CurrentValue);
			$detailUrl .= "&fk_bulan=" . urlencode($this->bulan->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "m_smplist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_smp`";
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		// Cascade Update detail table 'gaji_smp'
		$cascadeUpdate = FALSE;
		$rscascade = [];
		...
		if ($cascadeUpdate) {
			if (!isset($GLOBALS["gaji_smp"]))
				$GLOBALS["gaji_smp"] = new gaji_smp();
			$rswrk = $GLOBALS["gaji_smp"]->loadRs("`pid` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB') . " AND " . "`tahun` = " . QuotedValue($rsold['tahun'], DATATYPE_NUMBER, 'DB') . " AND " . "`bulan` = " . QuotedValue($rsold['bulan'], DATATYPE_NUMBER, 'DB'));
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["gaji_smp"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["gaji_smp"]->update($rscascade, $rskey, $rswrk->fields);
				if (!$success)
					return FALSE;

				// Call Row_Updated event
				$GLOBALS["gaji_smp"]->Row_Updated($rsdtlold, $rsdtlnew);
				$rswrk->moveNext();
			}
		}
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		// Cascade delete detail table 'gaji_smp'
		if (!isset($GLOBALS["gaji_smp"]))
			$GLOBALS["gaji_smp"] = new gaji_smp();
		$rscascade = $GLOBALS["gaji_smp"]->loadRs("`pid` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB") . " AND " . "`tahun` = " . QuotedValue($rs['tahun'], DATATYPE_NUMBER, "DB") . " AND " . "`bulan` = " . QuotedValue($rs['bulan'], DATATYPE_NUMBER, "DB"));
		$dtlrows = ($rscascade) ? $rscascade->getRows() : [];

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["gaji_smp"]->Row_Deleting($dtlrow);
			if (!$success)
				break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["gaji_smp"]->delete($dtlrow); // Delete
				if (!$success)
					break;
			}
		}
		...
		if ($success) {
			foreach ($dtlrows as $dtlrow)
				$GLOBALS["gaji_smp"]->Row_Deleted($dtlrow);
		}
		if ($success)
	...
	{
		global $Language;
		if ($pageName == "m_smpview.php")
			return $Language->phrase("View");
		elseif ($pageName == "m_smpedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_smpadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_smpview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_smpview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_smpadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_smpadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_smpedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_smpedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_smpadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_smpadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_smpdelete.php", $this->getUrlParm());
	}
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//echo "Row Inserted"
			...
			$tahun = $rsnew["tahun"];
			$bulan = $rsnew["bulan"];
			$row = ExecuteRows("SELECT * FROM pegawai where jenjang_id ='3' AND `type`= '1'");
			foreach($row as $query){
				...
				$myquery = "INSERT INTO gaji_smp VALUES (NULL,'".$query["nip"]."','".date('Y-m-d H:i:s')."','".date('Y-m-d')."',NULL,NULL,'".$query["jabatan"]."',NULL,'".$total."',NULL,NULL,NULL,NULL,NULL,NULL,NULL,'".$sub_total."','".$penyesuaian."','4','".$tambahan."','".$id."',NULL,NULL,NULL,NULL,NULL,NULL,NULL, NULL,NULL, NULL, '".$tahun."', '".$bulan."', NULL)";
	...
	function Row_Deleting(&$rs) {
		...
		$bulan = $rs["bulan"];
		$id = $rs["id"];
		$delete = ("DELETE FROM generate_pertahun_smp WHERE tahun ='".$tahun."' AND bulan ='".$bulan."' AND profesi ='1'");
		$delete_detil = ("DELETE FROM gaji_smp WHERE pid='".$id."'");
		return TRUE;
	}
