...
loadjs.ready("head", function() {
	...
	fpegawaiadd.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($pegawai_add->jjm->Required) { ?>
				elm = this.getElements("x" + infix + "_jjm");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pegawai_add->jjm->caption(), $pegawai_add->jjm->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_jjm");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($pegawai_add->jjm->errorMessage()) ?>");
			<?php if ($pegawai_add->status_peg->Required) { ?>
				elm = this.getElements("x" + infix + "_status_peg");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pegawai_add->status_peg->caption(), $pegawai_add->status_peg->RequiredErrorMessage)) ?>");
			<?php } ?>
			...
			<?php if ($pegawai_add->kehadiran->Required) { ?>
				elm = this.getElements("x" + infix + "_kehadiran");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pegawai_add->kehadiran->caption(), $pegawai_add->kehadiran->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_kehadiran");
				if (elm && !ew.checkInteger(elm.value))
	...
	fpegawaiadd.lists["x_status_peg"] = <?php echo $pegawai_add->status_peg->Lookup->toClientList($pegawai_add) ?>;
	fpegawaiadd.lists["x_status_peg"].options = <?php echo JsonEncode($pegawai_add->status_peg->lookupOptions()) ?>;
...
<?php } ?>
<?php if ($pegawai_add->jjm->Visible) { // jjm ?>
	<div id="r_jjm" class="form-group row">
		<label id="elh_pegawai_jjm" for="x_jjm" class="<?php echo $pegawai_add->LeftColumnClass ?>"><?php echo $pegawai_add->jjm->caption() ?><?php echo $pegawai_add->jjm->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pegawai_add->RightColumnClass ?>"><div <?php echo $pegawai_add->jjm->cellAttributes() ?>>
<span id="el_pegawai_jjm">
<input type="text" data-table="pegawai" data-field="x_jjm" name="x_jjm" id="x_jjm" size="30" maxlength="10" placeholder="<?php echo HtmlEncode($pegawai_add->jjm->getPlaceHolder()) ?>" value="<?php echo $pegawai_add->jjm->EditValue ?>"<?php echo $pegawai_add->jjm->editAttributes() ?>>
</span>
<?php echo $pegawai_add->jjm->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($pegawai_add->status_peg->Visible) { // status_peg ?>
	<div id="r_status_peg" class="form-group row">
		<label id="elh_pegawai_status_peg" for="x_status_peg" class="<?php echo $pegawai_add->LeftColumnClass ?>"><?php echo $pegawai_add->status_peg->caption() ?><?php echo $pegawai_add->status_peg->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pegawai_add->RightColumnClass ?>"><div <?php echo $pegawai_add->status_peg->cellAttributes() ?>>
<span id="el_pegawai_status_peg">
<div class="input-group ew-lookup-list">
	<div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_status_peg"><?php echo EmptyValue(strval($pegawai_add->status_peg->ViewValue)) ? $Language->phrase("PleaseSelect") : $pegawai_add->status_peg->ViewValue ?></div>
	<div class="input-group-append">
		<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($pegawai_add->status_peg->caption()), $Language->phrase("LookupLink", TRUE))) ?>" class="ew-lookup-btn btn btn-default"<?php echo ($pegawai_add->status_peg->ReadOnly || $pegawai_add->status_peg->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x_status_peg',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
	</div>
</div>
<?php echo $pegawai_add->status_peg->Lookup->getParamTag($pegawai_add, "p_x_status_peg") ?>
<input type="hidden" data-table="pegawai" data-field="x_status_peg" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $pegawai_add->status_peg->displayValueSeparatorAttribute() ?>" name="x_status_peg" id="x_status_peg" value="<?php echo $pegawai_add->status_peg->CurrentValue ?>"<?php echo $pegawai_add->status_peg->editAttributes() ?>>
</span>
<?php echo $pegawai_add->status_peg->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<input type="text" data-table="pegawai" data-field="x_mulai_bekerja" name="x_mulai_bekerja" id="x_mulai_bekerja" size="30" maxlength="4" placeholder="<?php echo HtmlEncode($pegawai_add->mulai_bekerja->getPlaceHolder()) ?>" value="<?php echo $pegawai_add->mulai_bekerja->EditValue ?>"<?php echo $pegawai_add->mulai_bekerja->editAttributes() ?>>
</span>
...
<?php } ?>
<?php if ($pegawai_add->kehadiran->Visible) { // kehadiran ?>
	<div id="r_kehadiran" class="form-group row">
		<label id="elh_pegawai_kehadiran" for="x_kehadiran" class="<?php echo $pegawai_add->LeftColumnClass ?>"><?php echo $pegawai_add->kehadiran->caption() ?><?php echo $pegawai_add->kehadiran->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pegawai_add->RightColumnClass ?>"><div <?php echo $pegawai_add->kehadiran->cellAttributes() ?>>
