...
loadjs.ready("head", function() {
	...
	fgajismk_detiledit.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($gajismk_detil_edit->jp->Required) { ?>
				elm = this.getElements("x" + infix + "_jp");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gajismk_detil_edit->jp->caption(), $gajismk_detil_edit->jp->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_jp");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($gajismk_detil_edit->jp->errorMessage()) ?>");
...
<?php } ?>
<?php if ($gajismk_detil_edit->jp->Visible) { // jp ?>
	<div id="r_jp" class="form-group row">
		<label id="elh_gajismk_detil_jp" for="x_jp" class="<?php echo $gajismk_detil_edit->LeftColumnClass ?>"><?php echo $gajismk_detil_edit->jp->caption() ?><?php echo $gajismk_detil_edit->jp->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $gajismk_detil_edit->RightColumnClass ?>"><div <?php echo $gajismk_detil_edit->jp->cellAttributes() ?>>
<span id="el_gajismk_detil_jp">
<input type="text" data-table="gajismk_detil" data-field="x_jp" name="x_jp" id="x_jp" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($gajismk_detil_edit->jp->getPlaceHolder()) ?>" value="<?php echo $gajismk_detil_edit->jp->EditValue ?>"<?php echo $gajismk_detil_edit->jp->editAttributes() ?>>
</span>
<?php echo $gajismk_detil_edit->jp->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
