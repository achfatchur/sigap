...
<?php if ($audittrail_list->field->Visible) { // field ?>
	...
	<?php } ?>
<?php } ?>
<?php if ($audittrail_list->keyvalue->Visible) { // keyvalue ?>
	<?php if ($audittrail_list->SortUrl($audittrail_list->keyvalue) == "") { ?>
		<th data-name="keyvalue" class="<?php echo $audittrail_list->keyvalue->headerCellClass() ?>"><div id="elh_audittrail_keyvalue" class="audittrail_keyvalue"><div class="ew-table-header-caption"><?php echo $audittrail_list->keyvalue->caption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="keyvalue" class="<?php echo $audittrail_list->keyvalue->headerCellClass() ?>"><div class="ew-pointer" onclick="ew.sort(event, '<?php echo $audittrail_list->SortUrl($audittrail_list->keyvalue) ?>', 1);"><div id="elh_audittrail_keyvalue" class="audittrail_keyvalue">
			<div class="ew-table-header-btn"><span class="ew-table-header-caption"><?php echo $audittrail_list->keyvalue->caption() ?><?php echo $Language->phrase("SrchLegend") ?></span><span class="ew-table-header-sort"><?php if ($audittrail_list->keyvalue->getSort() == "ASC") { ?><i class="fas fa-sort-up"></i><?php } elseif ($audittrail_list->keyvalue->getSort() == "DESC") { ?><i class="fas fa-sort-down"></i><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
<span<?php echo $audittrail_list->field->viewAttributes() ?>><?php echo $audittrail_list->field->getViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($audittrail_list->keyvalue->Visible) { // keyvalue ?>
		<td data-name="keyvalue" <?php echo $audittrail_list->keyvalue->cellAttributes() ?>>
<span id="el<?php echo $audittrail_list->RowCount ?>_audittrail_keyvalue">
<span<?php echo $audittrail_list->keyvalue->viewAttributes() ?>><?php echo $audittrail_list->keyvalue->getViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($audittrail_list->oldvalue->Visible) { // oldvalue ?>
