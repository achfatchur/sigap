...
loadjs.ready("head", function() {
	...
	fpotongan_smaadd.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php } ?>
			<?php if ($potongan_sma_add->tahun->Required) { ?>
				elm = this.getElements("x" + infix + "_tahun");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $potongan_sma_add->tahun->caption(), $potongan_sma_add->tahun->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_tahun");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($potongan_sma_add->tahun->errorMessage()) ?>");
			<?php if ($potongan_sma_add->bulan->Required) { ?>
				elm = this.getElements("x" + infix + "_bulan");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $potongan_sma_add->bulan->caption(), $potongan_sma_add->bulan->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_bulan");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($potongan_sma_add->bulan->errorMessage()) ?>");
			...
			<?php if ($potongan_sma_add->sertif->Required) { ?>
				elm = this.getElements("x" + infix + "_sertif");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $potongan_sma_add->sertif->caption(), $potongan_sma_add->sertif->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_sertif");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($potongan_sma_add->sertif->errorMessage()) ?>");
	...
	fpotongan_smaadd.lists["x_bulan"] = <?php echo $potongan_sma_add->bulan->Lookup->toClientList($potongan_sma_add) ?>;
	fpotongan_smaadd.lists["x_bulan"].options = <?php echo JsonEncode($potongan_sma_add->bulan->lookupOptions()) ?>;
	fpotongan_smaadd.autoSuggests["x_bulan"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
	...
	fpotongan_smaadd.lists["x_sertif"] = <?php echo $potongan_sma_add->sertif->Lookup->toClientList($potongan_sma_add) ?>;
	fpotongan_smaadd.lists["x_sertif"].options = <?php echo JsonEncode($potongan_sma_add->sertif->lookupOptions()) ?>;
	fpotongan_smaadd.autoSuggests["x_sertif"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
...
<input type="hidden" name="fk_id" value="<?php echo HtmlEncode($potongan_sma_add->pid->getSessionValue()) ?>">
<input type="hidden" name="fk_bulan" value="<?php echo HtmlEncode($potongan_sma_add->bulan->getSessionValue()) ?>">
<input type="hidden" name="fk_tahun" value="<?php echo HtmlEncode($potongan_sma_add->tahun->getSessionValue()) ?>">
<?php } ?>
<div class="ew-add-div"><!-- page* -->
<?php if ($potongan_sma_add->tahun->Visible) { // tahun ?>
	<div id="r_tahun" class="form-group row">
		<label id="elh_potongan_sma_tahun" for="x_tahun" class="<?php echo $potongan_sma_add->LeftColumnClass ?>"><?php echo $potongan_sma_add->tahun->caption() ?><?php echo $potongan_sma_add->tahun->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $potongan_sma_add->RightColumnClass ?>"><div <?php echo $potongan_sma_add->tahun->cellAttributes() ?>>
<?php if ($potongan_sma_add->tahun->getSessionValue() != "") { ?>
<span id="el_potongan_sma_tahun">
<span<?php echo $potongan_sma_add->tahun->viewAttributes() ?>><input type="text" readonly class="form-control-plaintext" value="<?php echo HtmlEncode(RemoveHtml($potongan_sma_add->tahun->ViewValue)) ?>"></span>
</span>
<input type="hidden" id="x_tahun" name="x_tahun" value="<?php echo HtmlEncode($potongan_sma_add->tahun->CurrentValue) ?>">
<?php } else { ?>
<span id="el_potongan_sma_tahun">
<input type="text" data-table="potongan_sma" data-field="x_tahun" name="x_tahun" id="x_tahun" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($potongan_sma_add->tahun->getPlaceHolder()) ?>" value="<?php echo $potongan_sma_add->tahun->EditValue ?>"<?php echo $potongan_sma_add->tahun->editAttributes() ?>>
</span>
<?php } ?>
<?php echo $potongan_sma_add->tahun->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($potongan_sma_add->bulan->Visible) { // bulan ?>
	<div id="r_bulan" class="form-group row">
		<label id="elh_potongan_sma_bulan" class="<?php echo $potongan_sma_add->LeftColumnClass ?>"><?php echo $potongan_sma_add->bulan->caption() ?><?php echo $potongan_sma_add->bulan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $potongan_sma_add->RightColumnClass ?>"><div <?php echo $potongan_sma_add->bulan->cellAttributes() ?>>
<?php if ($potongan_sma_add->bulan->getSessionValue() != "") { ?>
<span id="el_potongan_sma_bulan">
<span<?php echo $potongan_sma_add->bulan->viewAttributes() ?>><input type="text" readonly class="form-control-plaintext" value="<?php echo HtmlEncode(RemoveHtml($potongan_sma_add->bulan->ViewValue)) ?>"></span>
</span>
<input type="hidden" id="x_bulan" name="x_bulan" value="<?php echo HtmlEncode($potongan_sma_add->bulan->CurrentValue) ?>">
<?php } else { ?>
<span id="el_potongan_sma_bulan">
<?php
$onchange = $potongan_sma_add->bulan->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$potongan_sma_add->bulan->EditAttrs["onchange"] = "";
?>
<span id="as_x_bulan">
	<input type="text" class="form-control" name="sv_x_bulan" id="sv_x_bulan" value="<?php echo RemoveHtml($potongan_sma_add->bulan->EditValue) ?>" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($potongan_sma_add->bulan->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($potongan_sma_add->bulan->getPlaceHolder()) ?>"<?php echo $potongan_sma_add->bulan->editAttributes() ?>>
</span>
<input type="hidden" data-table="potongan_sma" data-field="x_bulan" data-value-separator="<?php echo $potongan_sma_add->bulan->displayValueSeparatorAttribute() ?>" name="x_bulan" id="x_bulan" value="<?php echo HtmlEncode($potongan_sma_add->bulan->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["fpotongan_smaadd"], function() {
	fpotongan_smaadd.createAutoSuggest({"id":"x_bulan","forceSelect":false});
});
</script>
<?php echo $potongan_sma_add->bulan->Lookup->getParamTag($potongan_sma_add, "p_x_bulan") ?>
</span>
<?php } ?>
<?php echo $potongan_sma_add->bulan->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php } ?>
<?php if ($potongan_sma_add->sertif->Visible) { // sertif ?>
	<div id="r_sertif" class="form-group row">
		<label id="elh_potongan_sma_sertif" class="<?php echo $potongan_sma_add->LeftColumnClass ?>"><?php echo $potongan_sma_add->sertif->caption() ?><?php echo $potongan_sma_add->sertif->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $potongan_sma_add->RightColumnClass ?>"><div <?php echo $potongan_sma_add->sertif->cellAttributes() ?>>
<span id="el_potongan_sma_sertif">
<?php
$onchange = $potongan_sma_add->sertif->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$potongan_sma_add->sertif->EditAttrs["onchange"] = "";
?>
<span id="as_x_sertif">
	<input type="text" class="form-control" name="sv_x_sertif" id="sv_x_sertif" value="<?php echo RemoveHtml($potongan_sma_add->sertif->EditValue) ?>" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($potongan_sma_add->sertif->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($potongan_sma_add->sertif->getPlaceHolder()) ?>"<?php echo $potongan_sma_add->sertif->editAttributes() ?>>
</span>
<input type="hidden" data-table="potongan_sma" data-field="x_sertif" data-value-separator="<?php echo $potongan_sma_add->sertif->displayValueSeparatorAttribute() ?>" name="x_sertif" id="x_sertif" value="<?php echo HtmlEncode($potongan_sma_add->sertif->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["fpotongan_smaadd"], function() {
	fpotongan_smaadd.createAutoSuggest({"id":"x_sertif","forceSelect":false});
});
</script>
<?php echo $potongan_sma_add->sertif->Lookup->getParamTag($potongan_sma_add, "p_x_sertif") ?>
</span>
<?php echo $potongan_sma_add->sertif->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php } ?>
	...
	<?php if (strval($potongan_sma_add->pid->getSessionValue()) != "") { ?>
	<input type="hidden" name="x_pid" id="x_pid" value="<?php echo HtmlEncode(strval($potongan_sma_add->pid->getSessionValue())) ?>">
	<?php } ?>
