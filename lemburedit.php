...
loadjs.ready("head", function() {
	...
	flemburedit.lists["x_pegawai"] = <?php echo $lembur_edit->pegawai->Lookup->toClientList($lembur_edit) ?>;
	flemburedit.lists["x_pegawai"].options = <?php echo JsonEncode($lembur_edit->pegawai->lookupOptions()) ?>;
...
<div class="input-group ew-lookup-list">
	<div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_pegawai"><?php echo EmptyValue(strval($lembur_edit->pegawai->ViewValue)) ? $Language->phrase("PleaseSelect") : $lembur_edit->pegawai->ViewValue ?></div>
	<div class="input-group-append">
		<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($lembur_edit->pegawai->caption()), $Language->phrase("LookupLink", TRUE))) ?>" class="ew-lookup-btn btn btn-default"<?php echo ($lembur_edit->pegawai->ReadOnly || $lembur_edit->pegawai->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x_pegawai',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
	</div>
</div>
<?php echo $lembur_edit->pegawai->Lookup->getParamTag($lembur_edit, "p_x_pegawai") ?>
<input type="hidden" data-table="lembur" data-field="x_pegawai" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $lembur_edit->pegawai->displayValueSeparatorAttribute() ?>" name="x_pegawai" id="x_pegawai" value="<?php echo $lembur_edit->pegawai->CurrentValue ?>"<?php echo $lembur_edit->pegawai->editAttributes() ?>>
</span>
