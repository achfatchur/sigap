...
loadjs.ready("head", function() {
	...
	fm_tidakhadiradd.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php } ?>
			<?php if ($m_tidakhadir_add->sertif->Required) { ?>
				elm = this.getElements("x" + infix + "_sertif");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $m_tidakhadir_add->sertif->caption(), $m_tidakhadir_add->sertif->RequiredErrorMessage)) ?>");
			<?php } ?>
			...
			<?php if ($m_tidakhadir_add->type->Required) { ?>
				elm = this.getElements("x" + infix + "_type");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $m_tidakhadir_add->type->caption(), $m_tidakhadir_add->type->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_type");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($m_tidakhadir_add->type->errorMessage()) ?>");
	...
	fm_tidakhadiradd.lists["x_sertif"] = <?php echo $m_tidakhadir_add->sertif->Lookup->toClientList($m_tidakhadir_add) ?>;
	fm_tidakhadiradd.lists["x_sertif"].options = <?php echo JsonEncode($m_tidakhadir_add->sertif->lookupOptions()) ?>;
...
<?php } ?>
<?php if ($m_tidakhadir_add->sertif->Visible) { // sertif ?>
	<div id="r_sertif" class="form-group row">
		<label id="elh_m_tidakhadir_sertif" for="x_sertif" class="<?php echo $m_tidakhadir_add->LeftColumnClass ?>"><?php echo $m_tidakhadir_add->sertif->caption() ?><?php echo $m_tidakhadir_add->sertif->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $m_tidakhadir_add->RightColumnClass ?>"><div <?php echo $m_tidakhadir_add->sertif->cellAttributes() ?>>
<span id="el_m_tidakhadir_sertif">
<div class="input-group ew-lookup-list">
	<div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_sertif"><?php echo EmptyValue(strval($m_tidakhadir_add->sertif->ViewValue)) ? $Language->phrase("PleaseSelect") : $m_tidakhadir_add->sertif->ViewValue ?></div>
	<div class="input-group-append">
		<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($m_tidakhadir_add->sertif->caption()), $Language->phrase("LookupLink", TRUE))) ?>" class="ew-lookup-btn btn btn-default"<?php echo ($m_tidakhadir_add->sertif->ReadOnly || $m_tidakhadir_add->sertif->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x_sertif',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
	</div>
</div>
<?php echo $m_tidakhadir_add->sertif->Lookup->getParamTag($m_tidakhadir_add, "p_x_sertif") ?>
<input type="hidden" data-table="m_tidakhadir" data-field="x_sertif" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $m_tidakhadir_add->sertif->displayValueSeparatorAttribute() ?>" name="x_sertif" id="x_sertif" value="<?php echo $m_tidakhadir_add->sertif->CurrentValue ?>"<?php echo $m_tidakhadir_add->sertif->editAttributes() ?>>
</span>
<?php echo $m_tidakhadir_add->sertif->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php } ?>
<?php if ($m_tidakhadir_add->type->Visible) { // type ?>
	<div id="r_type" class="form-group row">
		<label id="elh_m_tidakhadir_type" for="x_type" class="<?php echo $m_tidakhadir_add->LeftColumnClass ?>"><?php echo $m_tidakhadir_add->type->caption() ?><?php echo $m_tidakhadir_add->type->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $m_tidakhadir_add->RightColumnClass ?>"><div <?php echo $m_tidakhadir_add->type->cellAttributes() ?>>
<span id="el_m_tidakhadir_type">
<input type="text" data-table="m_tidakhadir" data-field="x_type" name="x_type" id="x_type" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($m_tidakhadir_add->type->getPlaceHolder()) ?>" value="<?php echo $m_tidakhadir_add->type->EditValue ?>"<?php echo $m_tidakhadir_add->type->editAttributes() ?>>
</span>
<?php echo $m_tidakhadir_add->type->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
