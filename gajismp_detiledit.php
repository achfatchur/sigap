...
loadjs.ready("head", function() {
	...
	fgajismp_detiledit.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($gajismp_detil_edit->jp->Required) { ?>
				elm = this.getElements("x" + infix + "_jp");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gajismp_detil_edit->jp->caption(), $gajismp_detil_edit->jp->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_jp");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($gajismp_detil_edit->jp->errorMessage()) ?>");
...
<?php } ?>
<?php if ($gajismp_detil_edit->jp->Visible) { // jp ?>
	<div id="r_jp" class="form-group row">
		<label id="elh_gajismp_detil_jp" for="x_jp" class="<?php echo $gajismp_detil_edit->LeftColumnClass ?>"><?php echo $gajismp_detil_edit->jp->caption() ?><?php echo $gajismp_detil_edit->jp->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $gajismp_detil_edit->RightColumnClass ?>"><div <?php echo $gajismp_detil_edit->jp->cellAttributes() ?>>
<span id="el_gajismp_detil_jp">
<input type="text" data-table="gajismp_detil" data-field="x_jp" name="x_jp" id="x_jp" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($gajismp_detil_edit->jp->getPlaceHolder()) ?>" value="<?php echo $gajismp_detil_edit->jp->EditValue ?>"<?php echo $gajismp_detil_edit->jp->editAttributes() ?>>
</span>
<?php echo $gajismp_detil_edit->jp->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
