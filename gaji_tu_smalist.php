...
<?php if ($gaji_tu_sma_list->voucher->Visible) { // voucher ?>
	...
	<?php } ?>
<?php } ?>
<?php if ($gaji_tu_sma_list->status->Visible) { // status ?>
	<?php if ($gaji_tu_sma_list->SortUrl($gaji_tu_sma_list->status) == "") { ?>
		<th data-name="status" class="<?php echo $gaji_tu_sma_list->status->headerCellClass() ?>"><div id="elh_gaji_tu_sma_status" class="gaji_tu_sma_status"><div class="ew-table-header-caption"><?php echo $gaji_tu_sma_list->status->caption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status" class="<?php echo $gaji_tu_sma_list->status->headerCellClass() ?>"><div class="ew-pointer" onclick="ew.sort(event, '<?php echo $gaji_tu_sma_list->SortUrl($gaji_tu_sma_list->status) ?>', 1);"><div id="elh_gaji_tu_sma_status" class="gaji_tu_sma_status">
			<div class="ew-table-header-btn"><span class="ew-table-header-caption"><?php echo $gaji_tu_sma_list->status->caption() ?></span><span class="ew-table-header-sort"><?php if ($gaji_tu_sma_list->status->getSort() == "ASC") { ?><i class="fas fa-sort-up"></i><?php } elseif ($gaji_tu_sma_list->status->getSort() == "DESC") { ?><i class="fas fa-sort-down"></i><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
<span<?php echo $gaji_tu_sma_list->voucher->viewAttributes() ?>><?php echo $gaji_tu_sma_list->voucher->getViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($gaji_tu_sma_list->status->Visible) { // status ?>
		<td data-name="status" <?php echo $gaji_tu_sma_list->status->cellAttributes() ?>>
<span id="el<?php echo $gaji_tu_sma_list->RowCount ?>_gaji_tu_sma_status">
<span<?php echo $gaji_tu_sma_list->status->viewAttributes() ?>><?php echo $gaji_tu_sma_list->status->getViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($gaji_tu_sma_list->potongan_bendahara->Visible) { // potongan_bendahara ?>
