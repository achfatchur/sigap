...
loadjs.ready("head", function() {
	...
	fpegawaiedit.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($pegawai_edit->jjm->Required) { ?>
				elm = this.getElements("x" + infix + "_jjm");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pegawai_edit->jjm->caption(), $pegawai_edit->jjm->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_jjm");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($pegawai_edit->jjm->errorMessage()) ?>");
			<?php if ($pegawai_edit->status_peg->Required) { ?>
				elm = this.getElements("x" + infix + "_status_peg");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pegawai_edit->status_peg->caption(), $pegawai_edit->status_peg->RequiredErrorMessage)) ?>");
			<?php } ?>
			...
			<?php if ($pegawai_edit->kehadiran->Required) { ?>
				elm = this.getElements("x" + infix + "_kehadiran");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pegawai_edit->kehadiran->caption(), $pegawai_edit->kehadiran->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_kehadiran");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($pegawai_edit->kehadiran->errorMessage()) ?>");
			<?php if ($pegawai_edit->status_pekerjaan->Required) { ?>
				elm = this.getElements("x" + infix + "_status_pekerjaan");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pegawai_edit->status_pekerjaan->caption(), $pegawai_edit->status_pekerjaan->RequiredErrorMessage)) ?>");
			<?php } ?>
			<?php if ($pegawai_edit->status_npwp->Required) { ?>
				elm = this.getElements("x" + infix + "_status_npwp");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pegawai_edit->status_npwp->caption(), $pegawai_edit->status_npwp->RequiredErrorMessage)) ?>");
			<?php } ?>
			<?php if ($pegawai_edit->bpjs_kesehatan->Required) { ?>
				elm = this.getElements("x" + infix + "_bpjs_kesehatan");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pegawai_edit->bpjs_kesehatan->caption(), $pegawai_edit->bpjs_kesehatan->RequiredErrorMessage)) ?>");
			<?php } ?>
	...
	fpegawaiedit.lists["x_status_peg"] = <?php echo $pegawai_edit->status_peg->Lookup->toClientList($pegawai_edit) ?>;
	fpegawaiedit.lists["x_status_peg"].options = <?php echo JsonEncode($pegawai_edit->status_peg->lookupOptions()) ?>;
	...
	fpegawaiedit.lists["x_status_pekerjaan"] = <?php echo $pegawai_edit->status_pekerjaan->Lookup->toClientList($pegawai_edit) ?>;
	fpegawaiedit.lists["x_status_pekerjaan"].options = <?php echo JsonEncode($pegawai_edit->status_pekerjaan->lookupOptions()) ?>;
	fpegawaiedit.lists["x_status_npwp"] = <?php echo $pegawai_edit->status_npwp->Lookup->toClientList($pegawai_edit) ?>;
	fpegawaiedit.lists["x_status_npwp"].options = <?php echo JsonEncode($pegawai_edit->status_npwp->lookupOptions()) ?>;
	fpegawaiedit.lists["x_bpjs_kesehatan"] = <?php echo $pegawai_edit->bpjs_kesehatan->Lookup->toClientList($pegawai_edit) ?>;
	fpegawaiedit.lists["x_bpjs_kesehatan"].options = <?php echo JsonEncode($pegawai_edit->bpjs_kesehatan->lookupOptions()) ?>;
...
<?php } ?>
<?php if ($pegawai_edit->jjm->Visible) { // jjm ?>
	<div id="r_jjm" class="form-group row">
		<label id="elh_pegawai_jjm" for="x_jjm" class="<?php echo $pegawai_edit->LeftColumnClass ?>"><?php echo $pegawai_edit->jjm->caption() ?><?php echo $pegawai_edit->jjm->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pegawai_edit->RightColumnClass ?>"><div <?php echo $pegawai_edit->jjm->cellAttributes() ?>>
<span id="el_pegawai_jjm">
<input type="text" data-table="pegawai" data-field="x_jjm" name="x_jjm" id="x_jjm" size="30" maxlength="10" placeholder="<?php echo HtmlEncode($pegawai_edit->jjm->getPlaceHolder()) ?>" value="<?php echo $pegawai_edit->jjm->EditValue ?>"<?php echo $pegawai_edit->jjm->editAttributes() ?>>
</span>
<?php echo $pegawai_edit->jjm->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($pegawai_edit->status_peg->Visible) { // status_peg ?>
	<div id="r_status_peg" class="form-group row">
		<label id="elh_pegawai_status_peg" for="x_status_peg" class="<?php echo $pegawai_edit->LeftColumnClass ?>"><?php echo $pegawai_edit->status_peg->caption() ?><?php echo $pegawai_edit->status_peg->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pegawai_edit->RightColumnClass ?>"><div <?php echo $pegawai_edit->status_peg->cellAttributes() ?>>
<span id="el_pegawai_status_peg">
<div class="input-group ew-lookup-list">
	<div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_status_peg"><?php echo EmptyValue(strval($pegawai_edit->status_peg->ViewValue)) ? $Language->phrase("PleaseSelect") : $pegawai_edit->status_peg->ViewValue ?></div>
	<div class="input-group-append">
		<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($pegawai_edit->status_peg->caption()), $Language->phrase("LookupLink", TRUE))) ?>" class="ew-lookup-btn btn btn-default"<?php echo ($pegawai_edit->status_peg->ReadOnly || $pegawai_edit->status_peg->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x_status_peg',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
	</div>
</div>
<?php echo $pegawai_edit->status_peg->Lookup->getParamTag($pegawai_edit, "p_x_status_peg") ?>
<input type="hidden" data-table="pegawai" data-field="x_status_peg" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $pegawai_edit->status_peg->displayValueSeparatorAttribute() ?>" name="x_status_peg" id="x_status_peg" value="<?php echo $pegawai_edit->status_peg->CurrentValue ?>"<?php echo $pegawai_edit->status_peg->editAttributes() ?>>
</span>
<?php echo $pegawai_edit->status_peg->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<input type="text" data-table="pegawai" data-field="x_mulai_bekerja" name="x_mulai_bekerja" id="x_mulai_bekerja" size="30" maxlength="4" placeholder="<?php echo HtmlEncode($pegawai_edit->mulai_bekerja->getPlaceHolder()) ?>" value="<?php echo $pegawai_edit->mulai_bekerja->EditValue ?>"<?php echo $pegawai_edit->mulai_bekerja->editAttributes() ?>>
</span>
...
<?php } ?>
<?php if ($pegawai_edit->kehadiran->Visible) { // kehadiran ?>
	<div id="r_kehadiran" class="form-group row">
		<label id="elh_pegawai_kehadiran" for="x_kehadiran" class="<?php echo $pegawai_edit->LeftColumnClass ?>"><?php echo $pegawai_edit->kehadiran->caption() ?><?php echo $pegawai_edit->kehadiran->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pegawai_edit->RightColumnClass ?>"><div <?php echo $pegawai_edit->kehadiran->cellAttributes() ?>>
<span id="el_pegawai_kehadiran">
<input type="text" data-table="pegawai" data-field="x_kehadiran" name="x_kehadiran" id="x_kehadiran" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($pegawai_edit->kehadiran->getPlaceHolder()) ?>" value="<?php echo $pegawai_edit->kehadiran->EditValue ?>"<?php echo $pegawai_edit->kehadiran->editAttributes() ?>>
</span>
<?php echo $pegawai_edit->kehadiran->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($pegawai_edit->status_pekerjaan->Visible) { // status_pekerjaan ?>
	<div id="r_status_pekerjaan" class="form-group row">
		<label id="elh_pegawai_status_pekerjaan" for="x_status_pekerjaan" class="<?php echo $pegawai_edit->LeftColumnClass ?>"><?php echo $pegawai_edit->status_pekerjaan->caption() ?><?php echo $pegawai_edit->status_pekerjaan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pegawai_edit->RightColumnClass ?>"><div <?php echo $pegawai_edit->status_pekerjaan->cellAttributes() ?>>
<span id="el_pegawai_status_pekerjaan">
<div class="input-group">
	<select class="custom-select ew-custom-select" data-table="pegawai" data-field="x_status_pekerjaan" data-value-separator="<?php echo $pegawai_edit->status_pekerjaan->displayValueSeparatorAttribute() ?>" id="x_status_pekerjaan" name="x_status_pekerjaan"<?php echo $pegawai_edit->status_pekerjaan->editAttributes() ?>>
			<?php echo $pegawai_edit->status_pekerjaan->selectOptionListHtml("x_status_pekerjaan") ?>
		</select>
</div>
<?php echo $pegawai_edit->status_pekerjaan->Lookup->getParamTag($pegawai_edit, "p_x_status_pekerjaan") ?>
</span>
<?php echo $pegawai_edit->status_pekerjaan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($pegawai_edit->status_npwp->Visible) { // status_npwp ?>
	<div id="r_status_npwp" class="form-group row">
		<label id="elh_pegawai_status_npwp" for="x_status_npwp" class="<?php echo $pegawai_edit->LeftColumnClass ?>"><?php echo $pegawai_edit->status_npwp->caption() ?><?php echo $pegawai_edit->status_npwp->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pegawai_edit->RightColumnClass ?>"><div <?php echo $pegawai_edit->status_npwp->cellAttributes() ?>>
<span id="el_pegawai_status_npwp">
<div class="input-group">
	<select class="custom-select ew-custom-select" data-table="pegawai" data-field="x_status_npwp" data-value-separator="<?php echo $pegawai_edit->status_npwp->displayValueSeparatorAttribute() ?>" id="x_status_npwp" name="x_status_npwp"<?php echo $pegawai_edit->status_npwp->editAttributes() ?>>
			<?php echo $pegawai_edit->status_npwp->selectOptionListHtml("x_status_npwp") ?>
		</select>
</div>
<?php echo $pegawai_edit->status_npwp->Lookup->getParamTag($pegawai_edit, "p_x_status_npwp") ?>
</span>
<?php echo $pegawai_edit->status_npwp->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($pegawai_edit->bpjs_kesehatan->Visible) { // bpjs_kesehatan ?>
	<div id="r_bpjs_kesehatan" class="form-group row">
		<label id="elh_pegawai_bpjs_kesehatan" for="x_bpjs_kesehatan" class="<?php echo $pegawai_edit->LeftColumnClass ?>"><?php echo $pegawai_edit->bpjs_kesehatan->caption() ?><?php echo $pegawai_edit->bpjs_kesehatan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pegawai_edit->RightColumnClass ?>"><div <?php echo $pegawai_edit->bpjs_kesehatan->cellAttributes() ?>>
<span id="el_pegawai_bpjs_kesehatan">
<div class="input-group">
	<select class="custom-select ew-custom-select" data-table="pegawai" data-field="x_bpjs_kesehatan" data-value-separator="<?php echo $pegawai_edit->bpjs_kesehatan->displayValueSeparatorAttribute() ?>" id="x_bpjs_kesehatan" name="x_bpjs_kesehatan"<?php echo $pegawai_edit->bpjs_kesehatan->editAttributes() ?>>
			<?php echo $pegawai_edit->bpjs_kesehatan->selectOptionListHtml("x_bpjs_kesehatan") ?>
		</select>
</div>
<?php echo $pegawai_edit->bpjs_kesehatan->Lookup->getParamTag($pegawai_edit, "p_x_bpjs_kesehatan") ?>
</span>
<?php echo $pegawai_edit->bpjs_kesehatan->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
