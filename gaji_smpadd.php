...
loadjs.ready("head", function() {
	...
	fgaji_smpadd.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($gaji_smp_add->tahun->Required) { ?>
				elm = this.getElements("x" + infix + "_tahun");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gaji_smp_add->tahun->caption(), $gaji_smp_add->tahun->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_tahun");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($gaji_smp_add->tahun->errorMessage()) ?>");
			<?php if ($gaji_smp_add->bulan->Required) { ?>
				elm = this.getElements("x" + infix + "_bulan");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gaji_smp_add->bulan->caption(), $gaji_smp_add->bulan->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_bulan");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($gaji_smp_add->bulan->errorMessage()) ?>");
			...
			<?php if ($gaji_smp_add->status->Required) { ?>
				elm = this.getElements("x" + infix + "_status");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gaji_smp_add->status->caption(), $gaji_smp_add->status->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_status");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($gaji_smp_add->status->errorMessage()) ?>");
			<?php if ($gaji_smp_add->potongan_bendahara->Required) { ?>
				elm = this.getElements("x" + infix + "_potongan_bendahara");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gaji_smp_add->potongan_bendahara->caption(), $gaji_smp_add->potongan_bendahara->RequiredErrorMessage)) ?>");
			<?php } ?>
			<?php if ($gaji_smp_add->voucher->Required) { ?>
				elm = this.getElements("x" + infix + "_voucher");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gaji_smp_add->voucher->caption(), $gaji_smp_add->voucher->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_voucher");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($gaji_smp_add->voucher->errorMessage()) ?>");
	...
	fgaji_smpadd.lists["x_bulan"] = <?php echo $gaji_smp_add->bulan->Lookup->toClientList($gaji_smp_add) ?>;
	fgaji_smpadd.lists["x_bulan"].options = <?php echo JsonEncode($gaji_smp_add->bulan->lookupOptions()) ?>;
	fgaji_smpadd.autoSuggests["x_bulan"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
...
<input type="hidden" name="fk_tahun" value="<?php echo HtmlEncode($gaji_smp_add->tahun->getSessionValue()) ?>">
<input type="hidden" name="fk_bulan" value="<?php echo HtmlEncode($gaji_smp_add->bulan->getSessionValue()) ?>">
<?php } ?>
<div class="ew-add-div"><!-- page* -->
<?php if ($gaji_smp_add->tahun->Visible) { // tahun ?>
	<div id="r_tahun" class="form-group row">
		<label id="elh_gaji_smp_tahun" for="x_tahun" class="<?php echo $gaji_smp_add->LeftColumnClass ?>"><?php echo $gaji_smp_add->tahun->caption() ?><?php echo $gaji_smp_add->tahun->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $gaji_smp_add->RightColumnClass ?>"><div <?php echo $gaji_smp_add->tahun->cellAttributes() ?>>
<?php if ($gaji_smp_add->tahun->getSessionValue() != "") { ?>
<span id="el_gaji_smp_tahun">
<span<?php echo $gaji_smp_add->tahun->viewAttributes() ?>><input type="text" readonly class="form-control-plaintext" value="<?php echo HtmlEncode(RemoveHtml($gaji_smp_add->tahun->ViewValue)) ?>"></span>
</span>
<input type="hidden" id="x_tahun" name="x_tahun" value="<?php echo HtmlEncode($gaji_smp_add->tahun->CurrentValue) ?>">
<?php } else { ?>
<span id="el_gaji_smp_tahun">
<input type="text" data-table="gaji_smp" data-field="x_tahun" name="x_tahun" id="x_tahun" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($gaji_smp_add->tahun->getPlaceHolder()) ?>" value="<?php echo $gaji_smp_add->tahun->EditValue ?>"<?php echo $gaji_smp_add->tahun->editAttributes() ?>>
</span>
<?php } ?>
<?php echo $gaji_smp_add->tahun->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($gaji_smp_add->bulan->Visible) { // bulan ?>
	<div id="r_bulan" class="form-group row">
		<label id="elh_gaji_smp_bulan" class="<?php echo $gaji_smp_add->LeftColumnClass ?>"><?php echo $gaji_smp_add->bulan->caption() ?><?php echo $gaji_smp_add->bulan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $gaji_smp_add->RightColumnClass ?>"><div <?php echo $gaji_smp_add->bulan->cellAttributes() ?>>
<?php if ($gaji_smp_add->bulan->getSessionValue() != "") { ?>
<span id="el_gaji_smp_bulan">
<span<?php echo $gaji_smp_add->bulan->viewAttributes() ?>><input type="text" readonly class="form-control-plaintext" value="<?php echo HtmlEncode(RemoveHtml($gaji_smp_add->bulan->ViewValue)) ?>"></span>
</span>
<input type="hidden" id="x_bulan" name="x_bulan" value="<?php echo HtmlEncode($gaji_smp_add->bulan->CurrentValue) ?>">
<?php } else { ?>
<span id="el_gaji_smp_bulan">
<?php
$onchange = $gaji_smp_add->bulan->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$gaji_smp_add->bulan->EditAttrs["onchange"] = "";
?>
<span id="as_x_bulan">
	<input type="text" class="form-control" name="sv_x_bulan" id="sv_x_bulan" value="<?php echo RemoveHtml($gaji_smp_add->bulan->EditValue) ?>" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($gaji_smp_add->bulan->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($gaji_smp_add->bulan->getPlaceHolder()) ?>"<?php echo $gaji_smp_add->bulan->editAttributes() ?>>
</span>
<input type="hidden" data-table="gaji_smp" data-field="x_bulan" data-value-separator="<?php echo $gaji_smp_add->bulan->displayValueSeparatorAttribute() ?>" name="x_bulan" id="x_bulan" value="<?php echo HtmlEncode($gaji_smp_add->bulan->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["fgaji_smpadd"], function() {
	fgaji_smpadd.createAutoSuggest({"id":"x_bulan","forceSelect":false});
});
</script>
<?php echo $gaji_smp_add->bulan->Lookup->getParamTag($gaji_smp_add, "p_x_bulan") ?>
</span>
<?php } ?>
<?php echo $gaji_smp_add->bulan->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php } ?>
<?php if ($gaji_smp_add->status->Visible) { // status ?>
	<div id="r_status" class="form-group row">
		<label id="elh_gaji_smp_status" for="x_status" class="<?php echo $gaji_smp_add->LeftColumnClass ?>"><?php echo $gaji_smp_add->status->caption() ?><?php echo $gaji_smp_add->status->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $gaji_smp_add->RightColumnClass ?>"><div <?php echo $gaji_smp_add->status->cellAttributes() ?>>
<span id="el_gaji_smp_status">
<input type="text" data-table="gaji_smp" data-field="x_status" name="x_status" id="x_status" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($gaji_smp_add->status->getPlaceHolder()) ?>" value="<?php echo $gaji_smp_add->status->EditValue ?>"<?php echo $gaji_smp_add->status->editAttributes() ?>>
</span>
<?php echo $gaji_smp_add->status->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($gaji_smp_add->potongan_bendahara->Visible) { // potongan_bendahara ?>
	<div id="r_potongan_bendahara" class="form-group row">
		<label id="elh_gaji_smp_potongan_bendahara" for="x_potongan_bendahara" class="<?php echo $gaji_smp_add->LeftColumnClass ?>"><?php echo $gaji_smp_add->potongan_bendahara->caption() ?><?php echo $gaji_smp_add->potongan_bendahara->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $gaji_smp_add->RightColumnClass ?>"><div <?php echo $gaji_smp_add->potongan_bendahara->cellAttributes() ?>>
<span id="el_gaji_smp_potongan_bendahara">
<textarea data-table="gaji_smp" data-field="x_potongan_bendahara" name="x_potongan_bendahara" id="x_potongan_bendahara" cols="35" rows="4" placeholder="<?php echo HtmlEncode($gaji_smp_add->potongan_bendahara->getPlaceHolder()) ?>"<?php echo $gaji_smp_add->potongan_bendahara->editAttributes() ?>><?php echo $gaji_smp_add->potongan_bendahara->EditValue ?></textarea>
</span>
<?php echo $gaji_smp_add->potongan_bendahara->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($gaji_smp_add->voucher->Visible) { // voucher ?>
	<div id="r_voucher" class="form-group row">
		<label id="elh_gaji_smp_voucher" for="x_voucher" class="<?php echo $gaji_smp_add->LeftColumnClass ?>"><?php echo $gaji_smp_add->voucher->caption() ?><?php echo $gaji_smp_add->voucher->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $gaji_smp_add->RightColumnClass ?>"><div <?php echo $gaji_smp_add->voucher->cellAttributes() ?>>
<span id="el_gaji_smp_voucher">
<input type="text" data-table="gaji_smp" data-field="x_voucher" name="x_voucher" id="x_voucher" size="30" maxlength="20" placeholder="<?php echo HtmlEncode($gaji_smp_add->voucher->getPlaceHolder()) ?>" value="<?php echo $gaji_smp_add->voucher->EditValue ?>"<?php echo $gaji_smp_add->voucher->editAttributes() ?>>
</span>
<?php echo $gaji_smp_add->voucher->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
