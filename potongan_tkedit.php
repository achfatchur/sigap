...
loadjs.ready("head", function() {
	...
	fpotongan_tkedit.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php } ?>
			<?php if ($potongan_tk_edit->tahun->Required) { ?>
				elm = this.getElements("x" + infix + "_tahun");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $potongan_tk_edit->tahun->caption(), $potongan_tk_edit->tahun->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_tahun");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($potongan_tk_edit->tahun->errorMessage()) ?>");
			<?php if ($potongan_tk_edit->bulan->Required) { ?>
				elm = this.getElements("x" + infix + "_bulan");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $potongan_tk_edit->bulan->caption(), $potongan_tk_edit->bulan->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_bulan");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($potongan_tk_edit->bulan->errorMessage()) ?>");
			...
			<?php if ($potongan_tk_edit->sertif->Required) { ?>
				elm = this.getElements("x" + infix + "_sertif");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $potongan_tk_edit->sertif->caption(), $potongan_tk_edit->sertif->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_sertif");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($potongan_tk_edit->sertif->errorMessage()) ?>");
	...
	fpotongan_tkedit.lists["x_bulan"] = <?php echo $potongan_tk_edit->bulan->Lookup->toClientList($potongan_tk_edit) ?>;
	fpotongan_tkedit.lists["x_bulan"].options = <?php echo JsonEncode($potongan_tk_edit->bulan->lookupOptions()) ?>;
	fpotongan_tkedit.autoSuggests["x_bulan"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
	fpotongan_tkedit.lists["x_nama"] = <?php echo $potongan_tk_edit->nama->Lookup->toClientList($potongan_tk_edit) ?>;
	fpotongan_tkedit.lists["x_nama"].options = <?php echo JsonEncode($potongan_tk_edit->nama->lookupOptions()) ?>;
	...
	fpotongan_tkedit.lists["x_sertif"] = <?php echo $potongan_tk_edit->sertif->Lookup->toClientList($potongan_tk_edit) ?>;
	fpotongan_tkedit.lists["x_sertif"].options = <?php echo JsonEncode($potongan_tk_edit->sertif->lookupOptions()) ?>;
	fpotongan_tkedit.autoSuggests["x_sertif"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
...
<input type="hidden" name="fk_id" value="<?php echo HtmlEncode($potongan_tk_edit->pid->getSessionValue()) ?>">
<input type="hidden" name="fk_bulan" value="<?php echo HtmlEncode($potongan_tk_edit->bulan->getSessionValue()) ?>">
<input type="hidden" name="fk_tahun" value="<?php echo HtmlEncode($potongan_tk_edit->tahun->getSessionValue()) ?>">
<?php } ?>
<div class="ew-edit-div"><!-- page* -->
<?php if ($potongan_tk_edit->tahun->Visible) { // tahun ?>
	<div id="r_tahun" class="form-group row">
		<label id="elh_potongan_tk_tahun" for="x_tahun" class="<?php echo $potongan_tk_edit->LeftColumnClass ?>"><?php echo $potongan_tk_edit->tahun->caption() ?><?php echo $potongan_tk_edit->tahun->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $potongan_tk_edit->RightColumnClass ?>"><div <?php echo $potongan_tk_edit->tahun->cellAttributes() ?>>
<?php if ($potongan_tk_edit->tahun->getSessionValue() != "") { ?>
<span id="el_potongan_tk_tahun">
<span<?php echo $potongan_tk_edit->tahun->viewAttributes() ?>><input type="text" readonly class="form-control-plaintext" value="<?php echo HtmlEncode(RemoveHtml($potongan_tk_edit->tahun->ViewValue)) ?>"></span>
</span>
<input type="hidden" id="x_tahun" name="x_tahun" value="<?php echo HtmlEncode($potongan_tk_edit->tahun->CurrentValue) ?>">
<?php } else { ?>
<span id="el_potongan_tk_tahun">
<input type="text" data-table="potongan_tk" data-field="x_tahun" name="x_tahun" id="x_tahun" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($potongan_tk_edit->tahun->getPlaceHolder()) ?>" value="<?php echo $potongan_tk_edit->tahun->EditValue ?>"<?php echo $potongan_tk_edit->tahun->editAttributes() ?>>
</span>
<?php } ?>
<?php echo $potongan_tk_edit->tahun->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($potongan_tk_edit->bulan->Visible) { // bulan ?>
	<div id="r_bulan" class="form-group row">
		<label id="elh_potongan_tk_bulan" class="<?php echo $potongan_tk_edit->LeftColumnClass ?>"><?php echo $potongan_tk_edit->bulan->caption() ?><?php echo $potongan_tk_edit->bulan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $potongan_tk_edit->RightColumnClass ?>"><div <?php echo $potongan_tk_edit->bulan->cellAttributes() ?>>
<?php if ($potongan_tk_edit->bulan->getSessionValue() != "") { ?>
<span id="el_potongan_tk_bulan">
<span<?php echo $potongan_tk_edit->bulan->viewAttributes() ?>><input type="text" readonly class="form-control-plaintext" value="<?php echo HtmlEncode(RemoveHtml($potongan_tk_edit->bulan->ViewValue)) ?>"></span>
</span>
<input type="hidden" id="x_bulan" name="x_bulan" value="<?php echo HtmlEncode($potongan_tk_edit->bulan->CurrentValue) ?>">
<?php } else { ?>
<span id="el_potongan_tk_bulan">
<?php
$onchange = $potongan_tk_edit->bulan->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$potongan_tk_edit->bulan->EditAttrs["onchange"] = "";
?>
<span id="as_x_bulan">
	<input type="text" class="form-control" name="sv_x_bulan" id="sv_x_bulan" value="<?php echo RemoveHtml($potongan_tk_edit->bulan->EditValue) ?>" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($potongan_tk_edit->bulan->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($potongan_tk_edit->bulan->getPlaceHolder()) ?>"<?php echo $potongan_tk_edit->bulan->editAttributes() ?>>
</span>
<input type="hidden" data-table="potongan_tk" data-field="x_bulan" data-value-separator="<?php echo $potongan_tk_edit->bulan->displayValueSeparatorAttribute() ?>" name="x_bulan" id="x_bulan" value="<?php echo HtmlEncode($potongan_tk_edit->bulan->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["fpotongan_tkedit"], function() {
	fpotongan_tkedit.createAutoSuggest({"id":"x_bulan","forceSelect":false});
});
</script>
<?php echo $potongan_tk_edit->bulan->Lookup->getParamTag($potongan_tk_edit, "p_x_bulan") ?>
</span>
<?php } ?>
<?php echo $potongan_tk_edit->bulan->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php $potongan_tk_edit->nama->EditAttrs->prepend("onchange", "ew.autoFill(this);"); ?>
<div class="input-group ew-lookup-list">
	<div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_nama"><?php echo EmptyValue(strval($potongan_tk_edit->nama->ViewValue)) ? $Language->phrase("PleaseSelect") : $potongan_tk_edit->nama->ViewValue ?></div>
	<div class="input-group-append">
		<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($potongan_tk_edit->nama->caption()), $Language->phrase("LookupLink", TRUE))) ?>" class="ew-lookup-btn btn btn-default"<?php echo ($potongan_tk_edit->nama->ReadOnly || $potongan_tk_edit->nama->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x_nama',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
	</div>
</div>
<?php echo $potongan_tk_edit->nama->Lookup->getParamTag($potongan_tk_edit, "p_x_nama") ?>
<input type="hidden" data-table="potongan_tk" data-field="x_nama" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $potongan_tk_edit->nama->displayValueSeparatorAttribute() ?>" name="x_nama" id="x_nama" value="<?php echo $potongan_tk_edit->nama->CurrentValue ?>"<?php echo $potongan_tk_edit->nama->editAttributes() ?>>
</span>
...
<?php } ?>
<?php if ($potongan_tk_edit->sertif->Visible) { // sertif ?>
	<div id="r_sertif" class="form-group row">
		<label id="elh_potongan_tk_sertif" class="<?php echo $potongan_tk_edit->LeftColumnClass ?>"><?php echo $potongan_tk_edit->sertif->caption() ?><?php echo $potongan_tk_edit->sertif->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $potongan_tk_edit->RightColumnClass ?>"><div <?php echo $potongan_tk_edit->sertif->cellAttributes() ?>>
<span id="el_potongan_tk_sertif">
<?php
$onchange = $potongan_tk_edit->sertif->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$potongan_tk_edit->sertif->EditAttrs["onchange"] = "";
?>
<span id="as_x_sertif">
	<input type="text" class="form-control" name="sv_x_sertif" id="sv_x_sertif" value="<?php echo RemoveHtml($potongan_tk_edit->sertif->EditValue) ?>" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($potongan_tk_edit->sertif->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($potongan_tk_edit->sertif->getPlaceHolder()) ?>"<?php echo $potongan_tk_edit->sertif->editAttributes() ?>>
</span>
<input type="hidden" data-table="potongan_tk" data-field="x_sertif" data-value-separator="<?php echo $potongan_tk_edit->sertif->displayValueSeparatorAttribute() ?>" name="x_sertif" id="x_sertif" value="<?php echo HtmlEncode($potongan_tk_edit->sertif->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["fpotongan_tkedit"], function() {
	fpotongan_tkedit.createAutoSuggest({"id":"x_sertif","forceSelect":false});
});
</script>
<?php echo $potongan_tk_edit->sertif->Lookup->getParamTag($potongan_tk_edit, "p_x_sertif") ?>
</span>
<?php echo $potongan_tk_edit->sertif->CustomMsg ?></div></div>
	</div>
<?php } ?>
