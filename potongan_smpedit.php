...
loadjs.ready("head", function() {
	...
	fpotongan_smpedit.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($potongan_smp_edit->tahun->Required) { ?>
				elm = this.getElements("x" + infix + "_tahun");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $potongan_smp_edit->tahun->caption(), $potongan_smp_edit->tahun->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_tahun");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($potongan_smp_edit->tahun->errorMessage()) ?>");
			<?php if ($potongan_smp_edit->bulan->Required) { ?>
				elm = this.getElements("x" + infix + "_bulan");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $potongan_smp_edit->bulan->caption(), $potongan_smp_edit->bulan->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_bulan");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($potongan_smp_edit->bulan->errorMessage()) ?>");
			...
			<?php if ($potongan_smp_edit->sertif->Required) { ?>
				elm = this.getElements("x" + infix + "_sertif");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $potongan_smp_edit->sertif->caption(), $potongan_smp_edit->sertif->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_sertif");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($potongan_smp_edit->sertif->errorMessage()) ?>");
	...
	fpotongan_smpedit.lists["x_bulan"] = <?php echo $potongan_smp_edit->bulan->Lookup->toClientList($potongan_smp_edit) ?>;
	fpotongan_smpedit.lists["x_bulan"].options = <?php echo JsonEncode($potongan_smp_edit->bulan->lookupOptions()) ?>;
	fpotongan_smpedit.autoSuggests["x_bulan"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
	...
	fpotongan_smpedit.lists["x_sertif"] = <?php echo $potongan_smp_edit->sertif->Lookup->toClientList($potongan_smp_edit) ?>;
	fpotongan_smpedit.lists["x_sertif"].options = <?php echo JsonEncode($potongan_smp_edit->sertif->lookupOptions()) ?>;
	fpotongan_smpedit.autoSuggests["x_sertif"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
...
<input type="hidden" name="fk_id" value="<?php echo HtmlEncode($potongan_smp_edit->pid->getSessionValue()) ?>">
<input type="hidden" name="fk_tahun" value="<?php echo HtmlEncode($potongan_smp_edit->tahun->getSessionValue()) ?>">
<input type="hidden" name="fk_bulan" value="<?php echo HtmlEncode($potongan_smp_edit->bulan->getSessionValue()) ?>">
<?php } ?>
...
<?php } ?>
<?php if ($potongan_smp_edit->tahun->Visible) { // tahun ?>
	<div id="r_tahun" class="form-group row">
		<label id="elh_potongan_smp_tahun" for="x_tahun" class="<?php echo $potongan_smp_edit->LeftColumnClass ?>"><?php echo $potongan_smp_edit->tahun->caption() ?><?php echo $potongan_smp_edit->tahun->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $potongan_smp_edit->RightColumnClass ?>"><div <?php echo $potongan_smp_edit->tahun->cellAttributes() ?>>
<?php if ($potongan_smp_edit->tahun->getSessionValue() != "") { ?>
<span id="el_potongan_smp_tahun">
<span<?php echo $potongan_smp_edit->tahun->viewAttributes() ?>><input type="text" readonly class="form-control-plaintext" value="<?php echo HtmlEncode(RemoveHtml($potongan_smp_edit->tahun->ViewValue)) ?>"></span>
</span>
<input type="hidden" id="x_tahun" name="x_tahun" value="<?php echo HtmlEncode($potongan_smp_edit->tahun->CurrentValue) ?>">
<?php } else { ?>
<span id="el_potongan_smp_tahun">
<input type="text" data-table="potongan_smp" data-field="x_tahun" name="x_tahun" id="x_tahun" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($potongan_smp_edit->tahun->getPlaceHolder()) ?>" value="<?php echo $potongan_smp_edit->tahun->EditValue ?>"<?php echo $potongan_smp_edit->tahun->editAttributes() ?>>
</span>
<?php } ?>
<?php echo $potongan_smp_edit->tahun->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($potongan_smp_edit->bulan->Visible) { // bulan ?>
	<div id="r_bulan" class="form-group row">
		<label id="elh_potongan_smp_bulan" class="<?php echo $potongan_smp_edit->LeftColumnClass ?>"><?php echo $potongan_smp_edit->bulan->caption() ?><?php echo $potongan_smp_edit->bulan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $potongan_smp_edit->RightColumnClass ?>"><div <?php echo $potongan_smp_edit->bulan->cellAttributes() ?>>
<?php if ($potongan_smp_edit->bulan->getSessionValue() != "") { ?>
<span id="el_potongan_smp_bulan">
<span<?php echo $potongan_smp_edit->bulan->viewAttributes() ?>><input type="text" readonly class="form-control-plaintext" value="<?php echo HtmlEncode(RemoveHtml($potongan_smp_edit->bulan->ViewValue)) ?>"></span>
</span>
<input type="hidden" id="x_bulan" name="x_bulan" value="<?php echo HtmlEncode($potongan_smp_edit->bulan->CurrentValue) ?>">
<?php } else { ?>
<span id="el_potongan_smp_bulan">
<?php
$onchange = $potongan_smp_edit->bulan->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$potongan_smp_edit->bulan->EditAttrs["onchange"] = "";
?>
<span id="as_x_bulan">
	<input type="text" class="form-control" name="sv_x_bulan" id="sv_x_bulan" value="<?php echo RemoveHtml($potongan_smp_edit->bulan->EditValue) ?>" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($potongan_smp_edit->bulan->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($potongan_smp_edit->bulan->getPlaceHolder()) ?>"<?php echo $potongan_smp_edit->bulan->editAttributes() ?>>
</span>
<input type="hidden" data-table="potongan_smp" data-field="x_bulan" data-value-separator="<?php echo $potongan_smp_edit->bulan->displayValueSeparatorAttribute() ?>" name="x_bulan" id="x_bulan" value="<?php echo HtmlEncode($potongan_smp_edit->bulan->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["fpotongan_smpedit"], function() {
	fpotongan_smpedit.createAutoSuggest({"id":"x_bulan","forceSelect":false});
});
</script>
<?php echo $potongan_smp_edit->bulan->Lookup->getParamTag($potongan_smp_edit, "p_x_bulan") ?>
</span>
<?php } ?>
<?php echo $potongan_smp_edit->bulan->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php } ?>
<?php if ($potongan_smp_edit->sertif->Visible) { // sertif ?>
	<div id="r_sertif" class="form-group row">
		<label id="elh_potongan_smp_sertif" class="<?php echo $potongan_smp_edit->LeftColumnClass ?>"><?php echo $potongan_smp_edit->sertif->caption() ?><?php echo $potongan_smp_edit->sertif->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $potongan_smp_edit->RightColumnClass ?>"><div <?php echo $potongan_smp_edit->sertif->cellAttributes() ?>>
<span id="el_potongan_smp_sertif">
<?php
$onchange = $potongan_smp_edit->sertif->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$potongan_smp_edit->sertif->EditAttrs["onchange"] = "";
?>
<span id="as_x_sertif">
	<input type="text" class="form-control" name="sv_x_sertif" id="sv_x_sertif" value="<?php echo RemoveHtml($potongan_smp_edit->sertif->EditValue) ?>" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($potongan_smp_edit->sertif->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($potongan_smp_edit->sertif->getPlaceHolder()) ?>"<?php echo $potongan_smp_edit->sertif->editAttributes() ?>>
</span>
<input type="hidden" data-table="potongan_smp" data-field="x_sertif" data-value-separator="<?php echo $potongan_smp_edit->sertif->displayValueSeparatorAttribute() ?>" name="x_sertif" id="x_sertif" value="<?php echo HtmlEncode($potongan_smp_edit->sertif->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["fpotongan_smpedit"], function() {
	fpotongan_smpedit.createAutoSuggest({"id":"x_sertif","forceSelect":false});
});
</script>
<?php echo $potongan_smp_edit->sertif->Lookup->getParamTag($potongan_smp_edit, "p_x_sertif") ?>
</span>
<?php echo $potongan_smp_edit->sertif->CustomMsg ?></div></div>
	</div>
<?php } ?>
