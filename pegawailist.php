...
loadjs.ready("head", function() {
	...
	fpegawailistsrch.validate = function(fobj) {
	...
	// Form_CustomValidate
	fpegawailistsrch.Form_CustomValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
	...
	fpegawailistsrch.validateRequired = <?php echo Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

	// Dynamic selection lists
	fpegawailistsrch.lists["x_jenjang_id"] = <?php echo $pegawai_list->jenjang_id->Lookup->toClientList($pegawai_list) ?>;
	fpegawailistsrch.lists["x_jenjang_id"].options = <?php echo JsonEncode($pegawai_list->jenjang_id->lookupOptions()) ?>;
...
// Render search row
$pegawai->RowType = ROWTYPE_SEARCH;
$pegawai->resetAttributes();
$pegawai_list->renderRow();
?>
<?php if ($pegawai_list->jenjang_id->Visible) { // jenjang_id ?>
	<?php
		$pegawai_list->SearchColumnCount++;
		if (($pegawai_list->SearchColumnCount - 1) % $pegawai_list->SearchFieldsPerRow == 0) {
			$pegawai_list->SearchRowCount++;
	?>
<div id="xsr_<?php echo $pegawai_list->SearchRowCount ?>" class="ew-row d-sm-flex">
	<?php
		}
	 ?>
	<div id="xsc_jenjang_id" class="ew-cell form-group">
		<label for="x_jenjang_id" class="ew-search-caption ew-label"><?php echo $pegawai_list->jenjang_id->caption() ?></label>
		<span class="ew-search-operator">
<?php echo $Language->phrase("=") ?>
<input type="hidden" name="z_jenjang_id" id="z_jenjang_id" value="=">
</span>
		<span id="el_pegawai_jenjang_id" class="ew-search-field">
<div class="input-group ew-lookup-list">
	<div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_jenjang_id"><?php echo EmptyValue(strval($pegawai_list->jenjang_id->AdvancedSearch->ViewValue)) ? $Language->phrase("PleaseSelect") : $pegawai_list->jenjang_id->AdvancedSearch->ViewValue ?></div>
	<div class="input-group-append">
		<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($pegawai_list->jenjang_id->caption()), $Language->phrase("LookupLink", TRUE))) ?>" class="ew-lookup-btn btn btn-default"<?php echo ($pegawai_list->jenjang_id->ReadOnly || $pegawai_list->jenjang_id->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x_jenjang_id',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
	</div>
</div>
<?php echo $pegawai_list->jenjang_id->Lookup->getParamTag($pegawai_list, "p_x_jenjang_id") ?>
<input type="hidden" data-table="pegawai" data-field="x_jenjang_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $pegawai_list->jenjang_id->displayValueSeparatorAttribute() ?>" name="x_jenjang_id" id="x_jenjang_id" value="<?php echo $pegawai_list->jenjang_id->AdvancedSearch->SearchValue ?>"<?php echo $pegawai_list->jenjang_id->editAttributes() ?>>
</span>
	</div>
	<?php if ($pegawai_list->SearchColumnCount % $pegawai_list->SearchFieldsPerRow == 0) { ?>
</div>
	<?php } ?>
<?php } ?>
	<?php if ($pegawai_list->SearchColumnCount % $pegawai_list->SearchFieldsPerRow > 0) { ?>
...
<?php if ($pegawai_list->periode_jabatan->Visible) { // periode_jabatan ?>
	...
	<?php } ?>
<?php } ?>
<?php if ($pegawai_list->jjm->Visible) { // jjm ?>
	<?php if ($pegawai_list->SortUrl($pegawai_list->jjm) == "") { ?>
		<th data-name="jjm" class="<?php echo $pegawai_list->jjm->headerCellClass() ?>"><div id="elh_pegawai_jjm" class="pegawai_jjm"><div class="ew-table-header-caption"><?php echo $pegawai_list->jjm->caption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="jjm" class="<?php echo $pegawai_list->jjm->headerCellClass() ?>"><div class="ew-pointer" onclick="ew.sort(event, '<?php echo $pegawai_list->SortUrl($pegawai_list->jjm) ?>', 1);"><div id="elh_pegawai_jjm" class="pegawai_jjm">
			<div class="ew-table-header-btn"><span class="ew-table-header-caption"><?php echo $pegawai_list->jjm->caption() ?></span><span class="ew-table-header-sort"><?php if ($pegawai_list->jjm->getSort() == "ASC") { ?><i class="fas fa-sort-up"></i><?php } elseif ($pegawai_list->jjm->getSort() == "DESC") { ?><i class="fas fa-sort-down"></i><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pegawai_list->status_peg->Visible) { // status_peg ?>
	<?php if ($pegawai_list->SortUrl($pegawai_list->status_peg) == "") { ?>
		<th data-name="status_peg" class="<?php echo $pegawai_list->status_peg->headerCellClass() ?>"><div id="elh_pegawai_status_peg" class="pegawai_status_peg"><div class="ew-table-header-caption"><?php echo $pegawai_list->status_peg->caption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status_peg" class="<?php echo $pegawai_list->status_peg->headerCellClass() ?>"><div class="ew-pointer" onclick="ew.sort(event, '<?php echo $pegawai_list->SortUrl($pegawai_list->status_peg) ?>', 1);"><div id="elh_pegawai_status_peg" class="pegawai_status_peg">
			<div class="ew-table-header-btn"><span class="ew-table-header-caption"><?php echo $pegawai_list->status_peg->caption() ?></span><span class="ew-table-header-sort"><?php if ($pegawai_list->status_peg->getSort() == "ASC") { ?><i class="fas fa-sort-up"></i><?php } elseif ($pegawai_list->status_peg->getSort() == "DESC") { ?><i class="fas fa-sort-down"></i><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
<?php if ($pegawai_list->aktif->Visible) { // aktif ?>
	...
	<?php } ?>
<?php } ?>
<?php if ($pegawai_list->kehadiran->Visible) { // kehadiran ?>
	<?php if ($pegawai_list->SortUrl($pegawai_list->kehadiran) == "") { ?>
		<th data-name="kehadiran" class="<?php echo $pegawai_list->kehadiran->headerCellClass() ?>"><div id="elh_pegawai_kehadiran" class="pegawai_kehadiran"><div class="ew-table-header-caption"><?php echo $pegawai_list->kehadiran->caption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="kehadiran" class="<?php echo $pegawai_list->kehadiran->headerCellClass() ?>"><div class="ew-pointer" onclick="ew.sort(event, '<?php echo $pegawai_list->SortUrl($pegawai_list->kehadiran) ?>', 1);"><div id="elh_pegawai_kehadiran" class="pegawai_kehadiran">
			<div class="ew-table-header-btn"><span class="ew-table-header-caption"><?php echo $pegawai_list->kehadiran->caption() ?></span><span class="ew-table-header-sort"><?php if ($pegawai_list->kehadiran->getSort() == "ASC") { ?><i class="fas fa-sort-up"></i><?php } elseif ($pegawai_list->kehadiran->getSort() == "DESC") { ?><i class="fas fa-sort-down"></i><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
<span<?php echo $pegawai_list->periode_jabatan->viewAttributes() ?>><?php echo $pegawai_list->periode_jabatan->getViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pegawai_list->jjm->Visible) { // jjm ?>
		<td data-name="jjm" <?php echo $pegawai_list->jjm->cellAttributes() ?>>
<span id="el<?php echo $pegawai_list->RowCount ?>_pegawai_jjm">
<span<?php echo $pegawai_list->jjm->viewAttributes() ?>><?php echo $pegawai_list->jjm->getViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($pegawai_list->status_peg->Visible) { // status_peg ?>
		<td data-name="status_peg" <?php echo $pegawai_list->status_peg->cellAttributes() ?>>
<span id="el<?php echo $pegawai_list->RowCount ?>_pegawai_status_peg">
<span<?php echo $pegawai_list->status_peg->viewAttributes() ?>><?php echo $pegawai_list->status_peg->getViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($pegawai_list->type->Visible) { // type ?>
