...
<div id="xsr_<?php echo $gajisd_detil_list->SearchRowCount ?>" class="ew-row d-sm-flex">
	...
	<div id="xsc_pegawai_id" class="ew-cell form-group">
		<label for="x_pegawai_id" class="ew-search-caption ew-label"><?php echo $gajisd_detil_list->pegawai_id->caption() ?></label>
...
<div class="input-group ew-lookup-list">
	<div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_pegawai_id"><?php echo EmptyValue(strval($gajisd_detil_list->pegawai_id->AdvancedSearch->ViewValue)) ? $Language->phrase("PleaseSelect") : $gajisd_detil_list->pegawai_id->AdvancedSearch->ViewValue ?></div>
	<div class="input-group-append">
		<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($gajisd_detil_list->pegawai_id->caption()), $Language->phrase("LookupLink", TRUE))) ?>" class="ew-lookup-btn btn btn-default"<?php echo ($gajisd_detil_list->pegawai_id->ReadOnly || $gajisd_detil_list->pegawai_id->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x_pegawai_id',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
	</div>
</div>
<?php echo $gajisd_detil_list->pegawai_id->Lookup->getParamTag($gajisd_detil_list, "p_x_pegawai_id") ?>
<input type="hidden" data-table="gajisd_detil" data-field="x_pegawai_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $gajisd_detil_list->pegawai_id->displayValueSeparatorAttribute() ?>" name="x_pegawai_id" id="x_pegawai_id" value="<?php echo $gajisd_detil_list->pegawai_id->AdvancedSearch->SearchValue ?>"<?php echo $gajisd_detil_list->pegawai_id->editAttributes() ?>>
</span>
	</div>
...
<?php if ($gajisd_detil_list->jumlahterima->Visible) { // jumlahterima ?>
	...
	<?php } ?>
<?php } ?>
<?php if ($gajisd_detil_list->jp->Visible) { // jp ?>
	<?php if ($gajisd_detil_list->SortUrl($gajisd_detil_list->jp) == "") { ?>
		<th data-name="jp" class="<?php echo $gajisd_detil_list->jp->headerCellClass() ?>"><div id="elh_gajisd_detil_jp" class="gajisd_detil_jp"><div class="ew-table-header-caption"><?php echo $gajisd_detil_list->jp->caption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="jp" class="<?php echo $gajisd_detil_list->jp->headerCellClass() ?>"><div class="ew-pointer" onclick="ew.sort(event, '<?php echo $gajisd_detil_list->SortUrl($gajisd_detil_list->jp) ?>', 1);"><div id="elh_gajisd_detil_jp" class="gajisd_detil_jp">
			<div class="ew-table-header-btn"><span class="ew-table-header-caption"><?php echo $gajisd_detil_list->jp->caption() ?></span><span class="ew-table-header-sort"><?php if ($gajisd_detil_list->jp->getSort() == "ASC") { ?><i class="fas fa-sort-up"></i><?php } elseif ($gajisd_detil_list->jp->getSort() == "DESC") { ?><i class="fas fa-sort-down"></i><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
<span<?php echo $gajisd_detil_list->jumlahterima->viewAttributes() ?>><?php echo $gajisd_detil_list->jumlahterima->getViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($gajisd_detil_list->jp->Visible) { // jp ?>
		<td data-name="jp" <?php echo $gajisd_detil_list->jp->cellAttributes() ?>>
<span id="el<?php echo $gajisd_detil_list->RowCount ?>_gajisd_detil_jp">
<span<?php echo $gajisd_detil_list->jp->viewAttributes() ?>><?php echo $gajisd_detil_list->jp->getViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
